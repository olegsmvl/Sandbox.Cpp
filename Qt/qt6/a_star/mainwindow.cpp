...
MainWindow::MainWindow(QWidget *parent)
  ...
  connect(ui->findPathButton, &QPushButton::clicked, this,
          &MainWindow::on_findPathButton_clicked);
...
void MainWindow::drawPath(const std::vector<Node> &path) {
  for (const Node &node : path) {
    scene->addRect(node.x * 20, node.y * 20, 20, 20, QPen(Qt::green),
                   QBrush(Qt::green));
  }
}
...
void MainWindow::on_findPathButton_clicked() {
  ...
  std::vector<Node> path =
      astar->findPath(0, 0, 19, 19); // Example start (0,0) and end (19,19)
  drawPath(path);
}

void MainWindow::on_generateButton_clicked() {
  scene->clear();
  astar->generate_obstacles();
  drawGrid();
}
