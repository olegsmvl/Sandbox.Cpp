...
/*
  ...
  [4] F3 for fast locating: TCAN TCANFD TLIN TFlexRay TEthernetHeader _TTSApp
  _TTSCOM _TTSTest
*/
...
#define ARRAY_PROPERTY(t, n, s)                                                \
  __declspec(property(put = property__set_##n, get = property__get_##n))       \
      t n[s];                                                                  \
  typedef t property__tmp_type_##n
#define READONLY_ARRAY_PROPERTY(t, n, s)                                       \
  __declspec(property(get = property__get_##n)) t n[s];                        \
  typedef t property__tmp_type_##n
#define WRITEONLY_ARRAY_PROPERTY(t, n, s)                                      \
  __declspec(property(put = property__set_##n)) t n[s];                        \
  typedef t property__tmp_type_##n
#define ARRAY_GET(n) property__tmp_type_##n property__get_##n(int index)
#define ARRAY_SET(n)                                                           \
  void property__set_##n(int index, const property__tmp_type_##n &value)

const u8 DLC_DATA_BYTE_CNT[16] = {0, 1,  2,  3,  4,  5,  6,  7,
...
typedef struct _TCAN {
  ...
  // is_tx -----------------------------------------------------
  PROPERTY(bool, is_tx);
  GET(is_tx) { return (FProperties & MASK_CANProp_DIR_TX) != 0; }
  SET(is_tx) {
    if (value) {
  ...
  GET(is_data) { return (FProperties & MASK_CANProp_REMOTE) == 0; }
  SET(is_data) {
    if (value) {
  ...
  GET(is_std) { return (FProperties & MASK_CANProp_EXTEND) == 0; }
  SET(is_std) {
    if (value) {
  ...
  GET(is_err) { return (FProperties & MASK_CANProp_ERROR) != 0; }
  SET(is_err) {
    if (value) {
  ...
  void set_data(const u8 d0, const u8 d1, const u8 d2, const u8 d3, const u8 d4,
                const u8 d5, const u8 d6, const u8 d7) {
    FData[0] = d0;
    FData[1] = d1;
    FData[2] = d2;
    FData[3] = d3;
...
typedef struct _TCANFD {
  ...
  GET(is_edl) { return (FFDProperties & MASK_CANFDProp_IS_FD) != 0; }
  SET(is_edl) {
    if (value) {
  ...
  GET(is_brs) { return (FFDProperties & MASK_CANFDProp_IS_BRS) != 0; }
  SET(is_brs) {
    if (value) {
  ...
  GET(is_esi) { return (FFDProperties & MASK_CANFDProp_IS_ESI) != 0; }
  SET(is_esi) {
    if (value) {
  ...
  // to CAN struct ---------------------------------------------
  TCAN to_tcan(void) { return *(TCAN *)(&FIdxChn); }
} TCANFD, *PCANFD;
...
typedef struct _LIN {
  ...
  // is_tx -----------------------------------------------------
  PROPERTY(bool, is_tx);
  GET(is_tx) { return (FProperties & MASK_LINProp_DIR_TX) != 0; }
  SET(is_tx) {
    if (value) {
...
typedef struct _TFlexRay {
  ...
  u8 FCCType; // 0 = Architecture independent, 1 = Invalid CC type, 2 = Cyclone
              // I, 3 = BUSDOCTOR, 4 = Cyclone II, 5 = Vector VN interface, 6 =
              // VN - Sync - Pulse(only in Status Event, for debugging purposes
              // only)
  ...
  // bit for the PDU was set in the received frame). 18
  // Reserved 19 1 = Raw frame(only valid if PDUs are used in
  // the configuration).A raw frame may contain PDUs in its
  // payload 20 1 = Dynamic segment    0 = Static segment 21
  // This flag is only valid for frames and not for PDUs.    1
  // = The PDUs in the payload of this frame are logged in
  // separate logging entries. 0 = The PDUs in the payload of
  // this frame must be extracted out of this frame.The logging
  // file does not contain separate  // PDU - entries. 22 Valid
  // for PDUs only.The bit is set if the PDU has an update bit
  ...
  GET(is_null) { return (FFrameFlags & ((u32)1 << 0)) != 0; }
  SET(is_null) {
    if (value) {
      FFrameFlags |= ((u32)1 << 0);
    } else {
  ...
  // is_data ---------------------------------------------------
  PROPERTY(bool, is_data);
  GET(is_data) { return (FFrameFlags & ((u32)1 << 1)) != 0; }
  SET(is_data) {
    if (value) {
      FFrameFlags |= ((u32)1 << 1);
    } else {
  ...
  GET(is_sync) { return (FFrameFlags & ((u32)1 << 2)) != 0; }
  SET(is_sync) {
    if (value) {
      FFrameFlags |= ((u32)1 << 2);
    } else {
  ...
  GET(is_startup) { return (FFrameFlags & ((u32)1 << 3)) != 0; }
  SET(is_startup) {
    if (value) {
      FFrameFlags |= ((u32)1 << 3);
    } else {
  ...
  // is_pp -----------------------------------------------------
  PROPERTY(bool, is_pp);
  GET(is_pp) { return (FFrameFlags & ((u32)1 << 4)) != 0; }
  SET(is_pp) {
    if (value) {
      FFrameFlags |= ((u32)1 << 4);
    } else {
  ...
  // is_err ----------------------------------------------------
  PROPERTY(bool, is_err);
  GET(is_err) { return (FFrameFlags & ((u32)1 << 6)) != 0; }
  SET(is_err) {
    if (value) {
      FFrameFlags |= ((u32)1 << 6);
    } else {
  ...
  GET(is_static_segment) { return (FFrameFlags & ((u32)1 << 20)) != 0; }
...
typedef struct _TEthernetHeader { // size = 24 B
  ...
  u8 FIdxPort;   // Network's switch's port index, 0~127: measurement port,
                 // 128~255: virtual port
  u8 FConfig;    // 0-1: 0 = Rx, 1 = Tx, 2 = TxRq
              // 2: crc status, for tx, 0: crc is include in data, 1: crc is not
              // include in data
              //                for rx, 0: crc is ok, 1: crc is not ok
              // 3: tx done type, 0: only report timestamp, 1: report full
              // info(header+frame)
  u16 FEthernetPayloadLength; // Length of Ethernet payload data in bytes. Max
                              // 1582 (1600 packet length - 18 header) data
                              // bytes per frame
  ...
  ARRAY_GET(Payloads) { return *(ethernet_payload_addr() + index); }
  ARRAY_SET(Payloads) { *(ethernet_payload_addr() + index) = value; }
  inline bool get_tx() { return (FConfig & 0x1) != 0; }
  ...
  bool is_virtual() { return FIdxPort >= 128; }
  pu16 first_vlan_addr() { return (pu16)(actual_data_pointer() + 6 + 6); }
  ...
  pu8 actual_data_pointer() { return &FIdxChn + sizeof(_TEthernetHeader); }
  ...
  pu8 destination_mac_addr() { return FEthernetDataAddr; }
  pu8 source_mac_addr() { return FEthernetDataAddr + 6; }
  ...
  u16 ethernet_type() { return *(pu16)(ethernet_type_addr()); }
  ...
  void set_ip_header_checksum(const u16 AValue) {
    ...
    if (!has_vlans(&o))
      o = 0;
    *(pu16)(FEthernetDataAddr + 0x18 + o) =
        (AValue >> 8) | ((AValue & 0xFF) << 8);
  }
...
typedef void(__stdcall *TProcedureSetDouble)(const void *AObj,
                                             const double AValue);
typedef double(__stdcall *TDoubleFunction)(const void *AObj);
typedef void(__stdcall *TProcedureSetString)(const void *AObj,
                                             const char *AValue);
...
typedef void(__stdcall *TProcedureSetCANFD)(const void *AObj,
                                            const PCANFD AValue);
...
typedef void(__stdcall *TWriteAPIDocumentFunc)(
    const void *AOpaque, const char *AName, const char *AGroup,
    const char *ADesc, const char *AExample, const s32 AParaCount);
typedef void(__stdcall *TWriteAPIParaFunc)(const void *AOpaque, const s32 AIdx,
                                           const char *AAPIName,
                                           const char *AParaName,
                                           const bool AIsConst,
                                           const char *AParaType,
                                           const char *ADesc);
...
typedef struct _TMPVarInt {
  ...
  s32 get(void) { return internal_get(FObj); }
  void set(const s32 AValue) { internal_set(FObj, AValue); }
} TMPVarInt;
...
typedef struct _TMPVarDouble {
  ...
  double get(void) { return internal_get(FObj); }
  void set(const double AValue) { internal_set(FObj, AValue); }
} TMPVarDouble;
...
typedef struct _TMPVarString {
  ...
  char *get(void) { return internal_get(FObj); }
  void set(const char *AValue) { internal_set(FObj, AValue); }
} TMPVarString;
...
typedef struct _TMPVarCAN {
  ...
  TCAN get(void) { return internal_get(FObj); }
  void set(TCAN AValue) { internal_set(FObj, &AValue); }
} TMPVarCAN;
...
typedef struct _TMPVarCANFD {
  ...
  TCANFD get(void) { return internal_get(FObj); }
  void set(TCANFD AValue) { internal_set(FObj, &AValue); }
} TMPVarCANFD;
...
typedef struct _TMPVarLIN {
  ...
  TLIN get(void) { return internal_get(FObj); }
  void set(TLIN AValue) { internal_set(FObj, &AValue); }
} TMPVarLIN;
...
typedef struct _TMPTimerMS {
  ...
  void start(void) { internal_start(FObj); }
  void stop(void) { internal_stop(FObj); }
  void set_interval(const s32 AInterval) {
    internal_set_interval(FObj, AInterval);
  }
  s32 get_interval(void) { return internal_get_interval(FObj); }
} TMPTimerMS;
...
typedef enum _TCANFDControllerType {
  fdtCAN = 0,
  fdtISOCANFD = 1,
  fdtNonISOCANFD = 2
} TCANFDControllerType;
typedef enum _TCANFDControllerMode {
  fdmNormal = 0,
  fdmACKOff = 1,
  fdmRestricted = 2
} TCANFDControllerMode;
// log def
typedef enum _TLIBOnlineReplayTimingMode {
  ortImmediately = 0,
  ortAsLog = 1,
  ortDelayed = 2
} TLIBOnlineReplayTimingMode;
typedef enum _TLIBOnlineReplayStatus {
  orsNotStarted = 0,
  orsRunning = 1,
  orsPaused = 2,
  orsCompleted = 3,
...
typedef enum _TSupportedBLFObjType {
  sotCAN = 0,
  sotLIN = 1,
  sotCANFD = 2,
...
typedef struct _TFlexRaySignal {
  ...
  u8 FCompuMethod; // 0 - Identical, 1 - Linear, 2 - Scale Linear, 3 -
                   // TextTable, 4 - TABNoIntp, 5 - Formula
  u8 FReserved;
  bool FIsIntel;
  s32 FStartBit;
  s32 FUpdateBit;
...
#define CANMsgDecl(typ, name, chn, prop, dlc, id)                              \
  const typ name = {{chn, prop, dlc, 0, id, 0, {0}}};
#define CANFDMsgDecl(typ, name, chn, prop, dlc, id)                            \
  const typ name = {{chn, prop, dlc, 1, id, 0, {0}}};
#define FlexRayFrameDecl(typ, name, chn, chnmask, prop, dlc, cycle, slot)      \
  const typ name = {{chn,                                                      \
                     chnmask,                                                  \
                     0,                                                        \
                     dlc,                                                      \
                     ...
                     0,                                                        \
                     {0}}};
#define LINMsgDecl(typ, name, chn, prop, dlc, id)                              \
  const typ name = {{chn, 0, prop, dlc, id, 0, 0, 0, {0}}};
#define CANSgnDecl(name, typ, isIntel, startBit, len, factor, offset)          \
  const TCANSignal name = {typ, isIntel, startBit, len, factor, offset};
#define LINSgnDecl(name, typ, isIntel, startBit, len, factor, offset)          \
  const TLINSignal name = {typ, isIntel, startBit, len, factor, offset};
#define FlexRaySgnDecl(name, typ, isIntel, compuMethod, startBit, updateBit,   \
                       len, factor, offset, actualStartBit, actualUpdateBit)   \
  const TFlexRaySignal name = {                                                \
      typ, compuMethod, 0,      isIntel,        startBit,       updateBit,     \
      len, factor,      offset, actualStartBit, actualUpdateBit};
typedef enum _TSignalType {
  stCANSignal,
  stLINSignal,
  stSystemVar,
  stFlexRay,
...
typedef enum _TSignalCheckKind {
  sckAlways,
  sckAppear,
  sckStatistics,
  sckRisingEdge,
...
typedef enum _TSignalStatisticsKind {
  sskMin,
  sskMax,
  sskAverage,
  sskStdDeviation
} TSignalStatisticsKind;
typedef enum _TLIBRBSInitValueOptions {
  rivUseDB = 0,
  rivUseLast,
  rivUse0
} TLIBRBSInitValueOptions;
typedef enum _TSymbolMappingDirection {
  smdBiDirection = 0,
  smdSgnToSysVar,
  smdSysVarToSgn
} TSymbolMappingDirection;
typedef void(__stdcall *TProgressCallback)(const void *AObj,
...
typedef void(__stdcall *TOnIoIPConnection)(const char *AIPAddress,
                                           const s32 APort);
// Automation Module - Graphic Program
typedef enum _TAutomationModuleRunningState {
  amrsNotRun,
  amrsPrepareRun,
  amrsRunning,
  amrsPaused,
  amrsStepping,
  amrsFinished
} TAutomationModuleRunningState,
    *PAutomationModuleRunningState;
typedef enum _TLIBAutomationSignalType {
  lastCANSignal,
  lastLINSignal,
  lastSysVar,
  lastLocalVar,
...
typedef enum _TSignalTesterFailReason {
  ...
  tfrJumpNotDetected
    ...
    *PSignalTesterFailReason;
typedef enum _TLIBMPFuncSource {
  lmfsSystemFunc,
  lmfsMPLib,
  lmfsInternal
} TLIBMPFuncSource;
typedef enum _TLIBSimVarType {
  lvtInteger,
  lvtDouble,
  lvtString,
  lvtCANMsg,
...
// stim
typedef enum _TSTIMSignalStatus {
  sssStopped,
  sssRunning,
  sssPaused
} TSTIMSignalStatus,
...
typedef s32(__stdcall *TTSDebugLog)(const void *AObj, const char *AFile,
                                    const char *AFunc, const s32 ALine,
                                    const char *AStr, const TLogLevel ALevel);
typedef s32(__stdcall *TTSAppSetTurboMode)(const bool AEnable);
typedef s32(__stdcall *TTSAppGetTurboMode)(const bool *AEnable);
typedef s32(__stdcall *TTSAppGetErrorDescription)(const s32 ACode,
                                                  char **ADesc);
typedef s32(__stdcall *TTSAppConfigureBaudrateCAN)(
    const s32 AIdxChn, const float ABaudrateKbps, const bool AListenOnly,
    const bool AInstallTermResistor120Ohm);
typedef s32(__stdcall *TTSAppConfigureBaudrateCANFD)(
    const s32 AIdxChn, const float ABaudrateArbKbps,
    const float ABaudrateDataKbps, const TCANFDControllerType AControllerType,
    const TCANFDControllerMode AControllerMode,
    const bool AInstallTermResistor120Ohm);
typedef s32(__stdcall *TTSAppTerminate)(const void *AObj);
typedef s32(__stdcall *TTSWaitTime)(const void *AObj, const s32 ATimeMs,
                                    const char *AMsg);
...
typedef s32(__stdcall *TTSShowConfirmDialog)(const char *ATitle,
                                             const char *APrompt,
                                             const char *AImage,
                                             const s32 ATimeoutMs,
                                             const bool ADefaultOK);
typedef s32(__stdcall *TTSPause)(void);
typedef s32(__stdcall *TTSSetCheckFailedTerminate)(const void *AObj,
                                                   const s32 AToTerminate);
typedef s32(__stdcall *TTSAppSplitString)(const char *ASplitter,
                                          const char *AStr, char **AArray,
                                          const s32 ASingleStrSize,
                                          const s32 AArraySize,
                                          s32 *AActualCount);
...
typedef s32(__stdcall *TTSAppSaveScreenshot)(const char *AFormCaption,
...
// system var def
typedef s32(__stdcall *TTSAppGetSystemVarCount)(s32 *AInternalCount,
                                                s32 *AUserCount);
typedef s32(__stdcall *TTSAppGetSystemVarDefByIndex)(
    const bool AIsUser, const s32 AIndex, const PLIBSystemVarDef AVarDef);
typedef s32(__stdcall *TTSAppFindSystemVarDefByName)(
    const bool AIsUser, const char *ACompleteName,
    const PLIBSystemVarDef AVarDef);
typedef s32(__stdcall *TTSAppCreateSystemVar)(const char *ACompleteName,
                                              const TLIBSystemVarType AType,
                                              const char *ADefaultValue,
                                              const char *AComment);
typedef s32(__stdcall *TTSAppDeleteSystemVar)(const char *ACompleteName);
typedef s32(__stdcall *TTSAppSetSystemVarUnit)(const char *ACompleteName,
                                               const char *AUnit);
typedef s32(__stdcall *TTSAppSetSystemVarValueTable)(const char *ACompleteName,
                                                     const char *ATable);
typedef s32(__stdcall *TTSAppGetSystemVarAddress)(const char *ACompleteName,
                                                  ps32 AAddress);
typedef s32(__stdcall *TTSAppSetSystemVarLogging)(const char *ACompleteName,
                                                  const bool AIsLogging);
typedef s32(__stdcall *TTSAppGetSystemVarLogging)(const char *ACompleteName,
                                                  bool *AIsLogging);
typedef s32(__stdcall *TTSAppLogSystemVarValue)(const void *AObj,
                                                const char *ACompleteName);
// system var get
typedef s32(__stdcall *TTSAppGetSystemVarDouble)(const char *ACompleteName,
                                                 double *AValue);
typedef s32(__stdcall *TTSAppGetSystemVarInt32)(const char *ACompleteName,
                                                s32 *AValue);
typedef s32(__stdcall *TTSAppGetSystemVarUInt32)(const char *ACompleteName,
                                                 u32 *AValue);
typedef s32(__stdcall *TTSAppGetSystemVarInt64)(const char *ACompleteName,
                                                s64 *AValue);
typedef s32(__stdcall *TTSAppGetSystemVarUInt64)(const char *ACompleteName,
                                                 u64 *AValue);
typedef s32(__stdcall *TTSAppGetSystemVarUInt8Array)(const char *ACompleteName,
                                                     const s32 AValueCapacity,
                                                     s32 *AVarCount,
                                                     u8 *AValue);
typedef s32(__stdcall *TTSAppGetSystemVarInt32Array)(const char *ACompleteName,
                                                     const s32 AValueCapacity,
                                                     s32 *AVarCount,
                                                     s32 *AValue);
typedef s32(__stdcall *TTSAppGetSystemVarInt64Array)(const char *ACompleteName,
                                                     const s32 AValueCapacity,
                                                     s32 *AVarCount,
                                                     s64 *AValue);
typedef s32(__stdcall *TTSAppGetSystemVarDoubleArray)(const char *ACompleteName,
                                                      const s32 AValueCapacity,
                                                      s32 *AVarCount,
                                                      double *AValue);
typedef s32(__stdcall *TTSAppGetSystemVarString)(const char *ACompleteName,
                                                 const s32 AValueCapacity,
                                                 char *AString);
typedef s32(__stdcall *TTSAppGetSystemVarGerneric)(const char *ACompleteName,
                                                   const s32 AValueCapacity,
                                                   char *AValue);
typedef s32(__stdcall *TTSWaitSystemVariable)(const char *ACompleteName,
                                              const char *AValue,
                                              const s32 ATimeoutMs);
// system var sync set
typedef s32(__stdcall *TTSAppSetSystemVarDouble)(const char *ACompleteName,
                                                 double AValue);
typedef s32(__stdcall *TTSAppSetSystemVarInt32)(const char *ACompleteName,
                                                s32 AValue);
typedef s32(__stdcall *TTSAppSetSystemVarUInt32)(const char *ACompleteName,
                                                 u32 AValue);
typedef s32(__stdcall *TTSAppSetSystemVarInt64)(const char *ACompleteName,
                                                s64 AValue);
typedef s32(__stdcall *TTSAppSetSystemVarUInt64)(const char *ACompleteName,
                                                 u64 AValue);
typedef s32(__stdcall *TTSAppSetSystemVarUInt8Array)(const char *ACompleteName,
                                                     const s32 ACount,
                                                     u8 *AValue);
typedef s32(__stdcall *TTSAppSetSystemVarInt32Array)(const char *ACompleteName,
                                                     const s32 ACount,
                                                     s32 *AValue);
typedef s32(__stdcall *TTSAppSetSystemVarInt64Array)(const char *ACompleteName,
                                                     const s32 ACount,
                                                     s64 *AValue);
typedef s32(__stdcall *TTSAppSetSystemVarDoubleArray)(const char *ACompleteName,
                                                      const s32 ACount,
                                                      double *AValue);
typedef s32(__stdcall *TTSAppSetSystemVarString)(const char *ACompleteName,
                                                 char *AString);
typedef s32(__stdcall *TTSAppSetSystemVarGeneric)(const char *ACompleteName,
                                                  char *AValue);
// system var async set
typedef s32(__stdcall *TTSAppSetSystemVarDoubleAsync)(const char *ACompleteName,
                                                      double AValue);
typedef s32(__stdcall *TTSAppSetSystemVarInt32Async)(const char *ACompleteName,
                                                     s32 AValue);
typedef s32(__stdcall *TTSAppSetSystemVarUInt32Async)(const char *ACompleteName,
                                                      u32 AValue);
typedef s32(__stdcall *TTSAppSetSystemVarInt64Async)(const char *ACompleteName,
                                                     s64 AValue);
typedef s32(__stdcall *TTSAppSetSystemVarUInt64Async)(const char *ACompleteName,
                                                      u64 AValue);
typedef s32(__stdcall *TTSAppSetSystemVarUInt8ArrayAsync)(
    const char *ACompleteName, const s32 ACount, u8 *AValue);
typedef s32(__stdcall *TTSAppSetSystemVarInt32ArrayAsync)(
    const char *ACompleteName, const s32 ACount, s32 *AValue);
typedef s32(__stdcall *TTSAppSetSystemVarInt64ArrayAsync)(
    const char *ACompleteName, const s32 ACount, s64 *AValue);
typedef s32(__stdcall *TTSAppSetSystemVarDoubleArrayAsync)(
    const char *ACompleteName, const s32 ACount, double *AValue);
typedef s32(__stdcall *TTSAppSetSystemVarStringAsync)(const char *ACompleteName,
                                                      char *AString);
typedef s32(__stdcall *TTSAppSetSystemVarGenericAsync)(
    const char *ACompleteName, char *AValue);
// system var utils
typedef s32(__stdcall *TTSAppLogSystemVar)(const char *ACompleteName);
typedef s32(__stdcall *TTSAppWaitSystemVarExistance)(const char *ACompleteName,
                                                     const s32 ATimeOutMs);
typedef s32(__stdcall *TTSAppWaitSystemVarDisappear)(const char *ACompleteName,
                                                     const s32 ATimeOutMs);
// excel
...
typedef s32(__stdcall *Texcel_set_sheet_count)(const void *AObj,
                                               const s32 ACount);
typedef s32(__stdcall *Texcel_get_sheet_name)(const void *AObj,
                                              const s32 AIdxSheet,
                                              char **AName);
typedef s32(__stdcall *Texcel_set_sheet_name)(const void *AObj,
                                              const s32 AIdxSheet,
                                              const char *AName);
typedef s32(__stdcall *Texcel_get_cell_count)(const void *AObj,
                                              const s32 AIdxSheet,
                                              s32 *ARowCount, s32 *AColCount);
typedef s32(__stdcall *Texcel_get_cell_value)(const void *AObj,
                                              const s32 AIdxSheet,
                                              const s32 AIdxRow,
                                              const s32 AIdxCol, char **AValue);
typedef s32(__stdcall *Texcel_set_cell_count)(const void *AObj,
                                              const s32 AIdxSheet,
                                              const s32 ARowCount,
                                              const s32 AColCount);
typedef s32(__stdcall *Texcel_set_cell_value)(const void *AObj,
                                              const s32 AIdxSheet,
                                              const s32 AIdxRow,
                                              const s32 AIdxCol, char *AValue);
...
// text file write
typedef s32(__stdcall *TWriteTextFileStart)(const char *AFileName,
                                            s32 *AHandle);
typedef s32(__stdcall *TWriteTextFileLine)(const s32 AHandle,
                                           const char *ALine);
typedef s32(__stdcall *TWriteTextFileLineWithDoubleArray)(const s32 AHandle,
                                                          const double *AArray,
                                                          const s32 ACount);
typedef s32(__stdcall *TWriteTextFileLineWithStringArray)(const s32 AHandle,
                                                          const char **AArray,
                                                          const s32 ACount);
...
typedef s32(__stdcall *TReadTextFileLine)(const s32 AHandle,
                                          const s32 ALineCapacity,
                                          ps32 AReadCharCount, char *ALine);
...
typedef s32(__stdcall *TWriteMatFileVariableDouble)(const s32 AHandle,
                                                    const char *AVarName,
                                                    const double AValue);
typedef s32(__stdcall *TWriteMatFileVariableString)(const s32 AHandle,
                                                    const char *AVarName,
                                                    const char *AValue);
typedef s32(__stdcall *TWriteMatFileVariableDoubleArray)(const s32 AHandle,
                                                         const char *AVarName,
                                                         const double *AArray,
                                                         const s32 ACount);
...
typedef s32(__stdcall *TReadMatFileVariableCount)(const s32 AHandle,
                                                  const char *AVarName,
                                                  ps32 ACount);
typedef s32(__stdcall *TReadMatFileVariableString)(const s32 AHandle,
                                                   const char *AVarName,
                                                   char *AValue,
                                                   const s32 AValueCapacity);
typedef s32(__stdcall *TReadMatFileVariableDouble)(const s32 AHandle,
                                                   const char *AVarName,
                                                   const double *AValue,
                                                   const s32 AStartIdx,
                                                   const s32 ACount);
...
typedef s32(__stdcall *TIniWriteInt32)(const s32 AHandle, const char *ASection,
                                       const char *AKey, const s32 AValue);
typedef s32(__stdcall *TIniWriteInt64)(const s32 AHandle, const char *ASection,
                                       const char *AKey, const s64 AValue);
typedef s32(__stdcall *TIniWriteBool)(const s32 AHandle, const char *ASection,
                                      const char *AKey, const bool AValue);
typedef s32(__stdcall *TIniWriteFloat)(const s32 AHandle, const char *ASection,
                                       const char *AKey, const double AValue);
typedef s32(__stdcall *TIniWriteString)(const s32 AHandle, const char *ASection,
                                        const char *AKey, const char *AValue);
typedef s32(__stdcall *TIniReadInt32)(const s32 AHandle, const char *ASection,
                                      const char *AKey, const s32 *AValue,
                                      const s32 ADefault);
typedef s32(__stdcall *TIniReadInt64)(const s32 AHandle, const char *ASection,
                                      const char *AKey, const s64 *AValue,
                                      const s64 ADefault);
typedef s32(__stdcall *TIniReadBool)(const s32 AHandle, const char *ASection,
                                     const char *AKey, const bool *AValue,
                                     const bool ADefault);
typedef s32(__stdcall *TIniReadFloat)(const s32 AHandle, const char *ASection,
                                      const char *AKey, const double *AValue,
                                      const double ADefault);
typedef s32(__stdcall *TIniReadString)(const s32 AHandle, const char *ASection,
                                       const char *AKey, const char *AValue,
                                       s32 *AValueCapacity,
                                       const char *ADefault);
typedef s32(__stdcall *TIniSectionExists)(const s32 AHandle,
                                          const char *ASection);
typedef s32(__stdcall *TIniKeyExists)(const s32 AHandle, const char *ASection,
                                      const char *AKey);
typedef s32(__stdcall *TIniDeleteKey)(const s32 AHandle, const char *ASection,
                                      const char *AKey);
typedef s32(__stdcall *TIniDeleteSection)(const s32 AHandle,
                                          const char *ASection);
typedef s32(__stdcall *TIniClose)(const s32 AHandle);
// automation module - Graphic Program
typedef s32(__stdcall *TAMGetRunningState)(const char *AModuleName,
                                           PAutomationModuleRunningState AState,
                                           char **ASubModuleName,
                                           char **ACurrentParameterGroupName);
typedef s32(__stdcall *TAMRun)(const char *AModuleName,
                               const char *ASubModuleName,
                               const char *AParameterGroupName,
                               const bool AIsSync);
typedef s32(__stdcall *TAMStop)(const char *AModuleName, const bool AIsSync);
typedef s32(__stdcall *TAMSelectSubModule)(const bool AIsSelect,
                                           const char *AModuleName,
                                           const char *ASubModuleName,
                                           const char *AParameterGroupName);
// panel set
typedef s32(__stdcall *TPanelSetEnable)(const char *APanelName,
                                        const char *AControlName,
                                        const bool AEnable);
typedef s32(__stdcall *TPanelSetPositionX)(const char *APanelName,
                                           const char *AControlName,
                                           const float AX);
typedef s32(__stdcall *TPanelSetPositionY)(const char *APanelName,
                                           const char *AControlName,
                                           const float AY);
typedef s32(__stdcall *TPanelSetPositionXY)(const char *APanelName,
                                            const char *AControlName,
                                            const float AX, const float AY);
typedef s32(__stdcall *TPanelSetOpacity)(const char *APanelName,
                                         const char *AControlName,
                                         const float AOpacity);
typedef s32(__stdcall *TPanelSetWidth)(const char *APanelName,
                                       const char *AControlName,
                                       const float AWidth);
typedef s32(__stdcall *TPanelSetHeight)(const char *APanelName,
                                        const char *AControlName,
                                        const float AHeight);
typedef s32(__stdcall *TPanelSetWidthHeight)(const char *APanelName,
                                             const char *AControlName,
                                             const float AWidth,
                                             const float AHeight);
typedef s32(__stdcall *TPanelSetRotationAngle)(const char *APanelName,
                                               const char *AControlName,
                                               const float AAngleDegree);
typedef s32(__stdcall *TPanelSetRotationCenter)(const char *APanelName,
                                                const char *AControlName,
                                                const float ARatioX,
                                                const float ARatioY);
typedef s32(__stdcall *TPanelSetScaleX)(const char *APanelName,
                                        const char *AControlName,
                                        const float AScaleX);
typedef s32(__stdcall *TPanelSetScaleY)(const char *APanelName,
                                        const char *AControlName,
                                        const float AScaleY);
typedef s32(__stdcall *TPanelSetBkgdColor)(const char *APanelName,
                                           const char *AControlName,
                                           const u32 AAlphaColor);
typedef s32(__stdcall *TPanelSetSelectorItems)(const char *APanelName,
                                               const char *AControlName,
                                               const char *AItemsList);
// panel get
typedef s32(__stdcall *TPanelGetEnable)(const char *APanelName,
                                        const char *AControlName,
                                        bool *AEnable);
typedef s32(__stdcall *TPanelGetPositionXY)(const char *APanelName,
                                            const char *AControlName, float *AX,
                                            float *AY);
typedef s32(__stdcall *TPanelGetOpacity)(const char *APanelName,
                                         const char *AControlName,
                                         float *AOpacity);
typedef s32(__stdcall *TPanelGetWidthHeight)(const char *APanelName,
                                             const char *AControlName,
                                             float *AWidth, float *AHeight);
typedef s32(__stdcall *TPanelGetRotationAngle)(const char *APanelName,
                                               const char *AControlName,
                                               float *AAngleDegree);
typedef s32(__stdcall *TPanelGetRotationCenter)(const char *APanelName,
                                                const char *AControlName,
                                                float *ARatioX, float *ARatioY);
typedef s32(__stdcall *TPanelGetScaleXY)(const char *APanelName,
                                         const char *AControlName,
                                         float *AScaleX, float *AScaleY);
typedef s32(__stdcall *TPanelGetBkgdColor)(const char *APanelName,
                                           const char *AControlName,
                                           pu32 AAlphaColor);
typedef s32(__stdcall *TPanelGetSelectorItems)(const char *APanelName,
                                               const char *AControlName,
                                               char **AItemsList);
// stim
typedef s32(__stdcall *TSTIMSetSignalStatus)(const char *ASTIMName,
                                             const char *AUserLabel,
                                             TSTIMSignalStatus AStatus);
typedef s32(__stdcall *TSTIMGetSignalStatus)(const char *ASTIMName,
                                             const char *AUserLabel,
                                             PSTIMSignalStatus AStatus);
// Atomic
typedef s32(__stdcall *TAtomicIncrement32)(const ps32 AAddr, const s32 AValue,
                                           ps32 AResult);
typedef s32(__stdcall *TAtomicIncrement64)(const ps64 AAddr, const s64 AValue,
                                           ps64 AResult);
...
// symbol mapping
typedef s32(__stdcall *TAddDirectMappingCAN)(
    const char *ADestinationVarName, const char *ASignalAddress,
    const TSymbolMappingDirection ADirection);
typedef s32(__stdcall *TAddDirectMappingWithFactorOffsetCAN)(
    const char *ADestinationVarName, const char *ASignalAddress,
    const TSymbolMappingDirection ADirection, const double AFactor,
    const double AOffset);
typedef s32(__stdcall *TAddExpressionMapping)(const char *ADestinationVarName,
                                              const char *AExpression,
                                              const char *AArguments);
typedef s32(__stdcall *TDeleteSymbolMappingItem)(
    const char *ADestinationVarName);
typedef s32(__stdcall *TDeleteSymbolMappingItems)(void);
typedef s32(__stdcall *TEnableSymbolMappingItem)(
    const char *ADestinationVarName, const bool AEnable);
...
typedef s32(__stdcall *TDBGetCANDBECUPropertiesByIndex)(
    PDBECUProperties AValue);
typedef s32(__stdcall *TDBGetLINDBECUPropertiesByIndex)(
    PDBECUProperties AValue);
typedef s32(__stdcall *TDBGetFlexRayDBECUPropertiesByIndex)(
    PDBECUProperties AValue);
typedef s32(__stdcall *TDBGetCANDBFramePropertiesByIndex)(
    PDBFrameProperties AValue);
typedef s32(__stdcall *TDBGetLINDBFramePropertiesByIndex)(
    PDBFrameProperties AValue);
typedef s32(__stdcall *TDBGetFlexRayDBFramePropertiesByIndex)(
    PDBFrameProperties AValue);
typedef s32(__stdcall *TDBGetCANDBSignalPropertiesByIndex)(
    PDBSignalProperties AValue);
typedef s32(__stdcall *TDBGetLINDBSignalPropertiesByIndex)(
    PDBSignalProperties AValue);
typedef s32(__stdcall *TDBGetFlexRayDBSignalPropertiesByIndex)(
    PDBSignalProperties AValue);
typedef s32(__stdcall *TDBGetCANDBPropertiesByAddress)(const char *AAddr,
                                                       PDBProperties AValue);
typedef s32(__stdcall *TDBGetLINDBPropertiesByAddress)(const char *AAddr,
                                                       PDBProperties AValue);
typedef s32(__stdcall *TDBGetFlexRayDBPropertiesByAddress)(
    const char *AAddr, PDBProperties AValue);
typedef s32(__stdcall *TDBGetCANDBECUPropertiesByAddress)(
    const char *AAddr, PDBECUProperties AValue);
typedef s32(__stdcall *TDBGetLINDBECUPropertiesByAddress)(
    const char *AAddr, PDBECUProperties AValue);
typedef s32(__stdcall *TDBGetFlexRayDBECUPropertiesByAddress)(
    const char *AAddr, PDBECUProperties AValue);
typedef s32(__stdcall *TDBGetCANDBFramePropertiesByAddress)(
    const char *AAddr, PDBFrameProperties AValue);
typedef s32(__stdcall *TDBGetLINDBFramePropertiesByAddress)(
    const char *AAddr, PDBFrameProperties AValue);
typedef s32(__stdcall *TDBGetFlexRayDBFramePropertiesByAddress)(
    const char *AAddr, PDBFrameProperties AValue);
typedef s32(__stdcall *TDBGetCANDBSignalPropertiesByAddress)(
    const char *AAddr, PDBSignalProperties AValue);
typedef s32(__stdcall *TDBGetLINDBSignalPropertiesByAddress)(
    const char *AAddr, PDBSignalProperties AValue);
typedef s32(__stdcall *TDBGetFlexRayDBSignalPropertiesByAddress)(
    const char *AAddr, PDBSignalProperties AValue);
typedef s32(__stdcall *TDBGetCANDBFramePropertiesByDBIndex)(
    const s32 AIdxDB, const s32 AIndex, PDBFrameProperties AValue);
typedef s32(__stdcall *TDBGetLINDBFramePropertiesByDBIndex)(
    const s32 AIdxDB, const s32 AIndex, PDBFrameProperties AValue);
typedef s32(__stdcall *TDBGetFlexRayDBFramePropertiesByDBIndex)(
    const s32 AIdxDB, const s32 AIndex, PDBFrameProperties AValue);
typedef s32(__stdcall *TDBGetCANDBSignalPropertiesByDBIndex)(
    const s32 AIdxDB, const s32 AIndex, PDBSignalProperties AValue);
typedef s32(__stdcall *TDBGetLINDBSignalPropertiesByDBIndex)(
    const s32 AIdxDB, const s32 AIndex, PDBSignalProperties AValue);
typedef s32(__stdcall *TDBGetFlexRayDBSignalPropertiesByDBIndex)(
    const s32 AIdxDB, const s32 AIndex, PDBSignalProperties AValue);
typedef s32(__stdcall *TDBGetCANDBSignalPropertiesByFrameIndex)(
    const s32 AIdxDB, const s32 AIdxFrame, const s32 ASgnIndexInFrame,
    PDBSignalProperties AValue);
typedef s32(__stdcall *TDBGetLINDBSignalPropertiesByFrameIndex)(
    const s32 AIdxDB, const s32 AIdxFrame, const s32 ASgnIndexInFrame,
    PDBSignalProperties AValue);
typedef s32(__stdcall *TDBGetFlexRayDBSignalPropertiesByFrameIndex)(
    const s32 AIdxDB, const s32 AIdxFrame, const s32 ASgnIndexInFrame,
    PDBSignalProperties AValue);
// misc
typedef s32(__stdcall *TTSAppMakeToast)(const char *AString,
                                        const TLogLevel ALevel);
typedef s32(__stdcall *TTSAppMakeToastUntil)(const char *AString,
                                             const TLogLevel ALevel,
                                             const bool *ACloseCriteria,
                                             const bool AUserCanBreak);
typedef s32(__stdcall *TTSAppMakeToastWithCallback)(
    const char *AString, const TLogLevel ALevel,
    const TCheckResultCallback ACallback, const bool AUserCanBreak);
typedef s32(__stdcall *TTSAppExecutePythonString)(const char *AString,
                                                  const char *AArguments,
                                                  const bool AIsSync,
                                                  const bool AIsX64,
                                                  char **AResultLog);
typedef s32(__stdcall *TTSAppExecutePythonScript)(const char *AFilePath,
                                                  const char *AArguments,
                                                  const bool AIsSync,
                                                  const bool AIsX64,
                                                  char **AResultLog);
typedef s32(__stdcall *TTSAppExecuteApp)(const char *AAppPath,
                                         const char *AWorkingDir,
                                         const char *AParameter,
                                         const s32 AWaitTimeMS);
typedef s32(__stdcall *TTSAppTerminateAppByName)(const char *AImageName);
typedef s32(__stdcall *TTSAppCallMPAPI)(const char *ALibName,
                                        const char *AFuncName,
                                        const char *AInParameters,
                                        char **AOutParameters);
typedef s32(__stdcall *TTSAppSetAnalysisTimeRange)(const s64 ATimeStartUs,
                                                   const s64 ATimeEndUs);
typedef s32(__stdcall *TTSAppEnableAllGraphics)(const bool AEnable,
                                                const char *AExceptCaptions);
typedef s32(__stdcall *TTSAppGetTSMasterVersion)(ps32 AYear, ps32 AMonth,
                                                 ps32 ADay, ps32 ABuildNumber);
typedef s32(__stdcall *TUIShowPageByIndex)(const s32 AIndex);
typedef s32(__stdcall *TUIShowPageByName)(const char *AName);
typedef s32(__stdcall *TWriteRealtimeComment)(const void *AObj,
                                              const char *AName);
typedef s32(__stdcall *TTSAppSetThreadPriority)(const void *AObj,
                                                const s32 APriority);
...
typedef s32(__stdcall *TPromptUserInputValue)(const char *APrompt,
                                              double *AValue);
typedef s32(__stdcall *TPromptUserInputString)(const char *APrompt,
                                               char *AValue,
                                               const s32 AValueCapacity);
...
typedef s32(__stdcall *TPlaySound)(const bool AIsSync,
                                   const char *AWaveFileName);
...
typedef s32(__stdcall *TWaitWithDialog)(const void *AObj, const char *ATitle,
                                        const char *AMessage,
                                        const bool *ApResult,
                                        const float *ApProgress100);
typedef s32(__cdecl *TRunPythonFunction)(const void *AObj,
                                         const char *AModuleName,
                                         const char *AFuncName,
                                         const char *AArgFormat, ...);
typedef char *(__stdcall *TGetCurrentMpName)(const void *AObj);
typedef s32(__stdcall *TGetSystemConstantCount)(const s32 AIdxType,
                                                ps32 ACount);
typedef s32(__stdcall *TGetSystemConstantValueByIndex)(const s32 AIdxType,
                                                       const s32 AIdxValue,
                                                       char **AName,
                                                       pdouble AValue,
                                                       char **ADescription);
typedef s32(__stdcall *TAddSystemConstant)(const char *AName,
                                           const double AValue,
                                           const char *ADescription);
typedef s32(__stdcall *TDeleteSystemConstant)(const char *AName);
typedef s32(__stdcall *TDBGetFlexRayClusterParameters)(
    const s32 AIdxChn, const pchar AClusterName,
    PFlexRayClusterParameters AValue);
typedef s32(__stdcall *TDBGetFlexRayControllerParameters)(
    const s32 AIdxChn, pchar AClusterName, const pchar AECUName,
    PFlexRayControllerParameters AValue);
typedef s32(__stdcall *TSetSystemVarEventSupport)(const pchar ACompleteName,
                                                  const bool ASupport);
typedef s32(__stdcall *TGetSystemVarEventSupport)(const pchar ACompleteName,
                                                  pbool ASupport);
typedef s32(__stdcall *TGetDateTime)(ps32 AYear, ps32 AMonth, ps32 ADay,
                                     ps32 AHour, ps32 AMinute, ps32 ASecond,
                                     ps32 AMilliseconds);
typedef s32(__stdcall *TGPGDeleteAllModules)(void);
typedef s32(__stdcall *TGPGCreateModule)(const pchar AProgramName,
                                         const pchar ADisplayName,
                                         ps64 AModuleId, ps64 AEntryPointId);
typedef s32(__stdcall *TGPGDeleteModule)(const s64 AModuleId);
typedef s32(__stdcall *TGPGDeployModule)(
    const s64 AModuleId, const pchar AGraphicProgramWindowTitle);
typedef s32(__stdcall *TGPGAddActionDown)(const s64 AModuleId,
                                          const s64 AUpperActionId,
                                          const pchar ADisplayName,
                                          const pchar AComment, ps64 AActionId);
typedef s32(__stdcall *TGPGAddActionRight)(const s64 AModuleId,
                                           const s64 ALeftActionId,
                                           const pchar ADisplayName,
                                           const pchar AComment,
                                           ps64 AActionId);
typedef s32(__stdcall *TGPGAddGoToDown)(const s64 AModuleId,
                                        const s64 AUpperActionId,
                                        const pchar ADisplayName,
                                        const pchar AComment,
                                        const pchar AJumpLabel, ps64 AActionId);
typedef s32(__stdcall *TGPGAddGoToRight)(
    const s64 AModuleId, const s64 ALeftActionId, const pchar ADisplayName,
    const pchar AComment, const pchar AJumpLabel, ps64 AActionId);
typedef s32(__stdcall *TGPGAddFromDown)(const s64 AModuleId,
                                        const s64 AUpperActionId,
                                        const pchar ADisplayName,
                                        const pchar AComment,
                                        const pchar AJumpLabel, ps64 AActionId);
typedef s32(__stdcall *TGPGAddGroupDown)(const s64 AModuleId,
                                         const s64 AUpperActionId,
                                         const pchar ADisplayName,
                                         const pchar AComment, ps64 AGroupId,
                                         ps64 AEntryPointId);
typedef s32(__stdcall *TGPGAddGroupRight)(const s64 AModuleId,
                                          const s64 ALeftActionId,
                                          const pchar ADisplayName,
                                          const pchar AComment, ps64 AGroupId,
                                          ps64 AEntryPointId);
typedef s32(__stdcall *TGPGDeleteAction)(const s64 AModuleId,
                                         const s64 AActionId);
typedef s32(__stdcall *TGPGSetActionNOP)(const s64 AModuleId,
                                         const s64 AActionId);
typedef s32(__stdcall *TGPGSetActionSignalReadWrite)(const s64 AModuleId,
                                                     const s64 AActionId);
typedef s32(__stdcall *TGPGSetActionAPICall)(const s64 AModuleId,
                                             const s64 AActionId);
typedef s32(__stdcall *TGPGSetActionExpression)(const s64 AModuleId,
                                                const s64 AActionId);
typedef s32(__stdcall *TGPGConfigureActionBasic)(const s64 AModuleId,
                                                 const s64 AActionId,
                                                 const pchar ADisplayName,
                                                 const pchar AComment,
                                                 const s32 ATimeoutMs);
typedef s32(__stdcall *TGPGConfigureGoTo)(const s64 AModuleId,
                                          const s64 AActionId,
                                          const pchar ADisplayName,
                                          const pchar AComment,
                                          const pchar AJumpLabel);
typedef s32(__stdcall *TGPGConfigureFrom)(const s64 AModuleId,
                                          const s64 AActionId,
                                          const pchar ADisplayName,
                                          const pchar AComment,
                                          const pchar AJumpLabel);
typedef s32(__stdcall *TGPGConfigureNOP)(const s64 AModuleId,
                                         const s64 AActionId,
                                         const bool ANextDirectionIsDown,
                                         const bool AResultOK,
                                         const bool AJumpBackIfEnded);
typedef s32(__stdcall *TGPGConfigureGroup)(
    const s64 AModuleId, const s64 AActionId,
    const TLIBAutomationSignalType ARepeatCountType,
    const pchar ARepeatCountRepr);
typedef s32(__stdcall *TGPGConfigureSignalReadWriteListClear)(
    const s64 AModuleId, const s64 AActionId);
typedef s32(__stdcall *TGPGConfigureSignalWriteListAppend)(
    const s64 AModuleId, const s64 AActionId,
    const TLIBAutomationSignalType ADestSignalType,
    const TLIBAutomationSignalType ASrcSignalType, const pchar ADestSignalExpr,
    const pchar ASrcSignalExpr, ps32 AItemIndex);
typedef s32(__stdcall *TGPGConfigureSignalReadListAppend)(
    const s64 AModuleId, const s64 AActionId, const bool AIsConditionAND,
    const TLIBAutomationSignalType ADestSignalType,
    const TLIBAutomationSignalType AMinSignalType,
    const TLIBAutomationSignalType AMaxSignalType, const pchar ADestSignalExpr,
    const pchar AMinSignalExpr, const pchar AMaxSignalExpr, ps32 AItemIndex);
typedef s32(__stdcall *TGPGConfigureAPICallArguments)(
    const s64 AModuleId, const s64 AActionId, const TLIBMPFuncSource AAPIType,
    const pchar AAPIName, const PLIBAutomationSignalType AAPIArgTypes,
    char **AAPIArgNames, char **AAPIArgExprs, const s32 AArraySize);
typedef s32(__stdcall *TGPGConfigureAPICallResult)(
    const s64 AModuleId, const s64 AActionId, const bool AIgnoreResult,
    const TLIBAutomationSignalType ASignalType, const pchar ASignalExpr);
typedef s32(__stdcall *TGPGConfigureExpression)(
    const s64 AModuleId, const s64 AActionId, const s32 AxCount,
    const pchar AExpression, const PLIBAutomationSignalType AArgumentTypes,
    char **AArgumentExprs, const TLIBAutomationSignalType AResultType,
    const pchar AResultExpr);
typedef s32(__stdcall *TGPGAddLocalVar)(const s64 AModuleId,
                                        const TLIBSimVarType AType,
                                        const pchar AName,
                                        const pchar AInitValue,
                                        const pchar AComment, ps32 AItemIndex);
typedef s32(__stdcall *TGPGDeleteLocalVar)(const s64 AModuleId,
                                           const s32 AItemIndex);
typedef s32(__stdcall *TGPGDeleteAllLoalVars)(const s64 AModuleId);
typedef s32(__stdcall *TGPGDeleteGroupItems)(const s64 AModuleId,
                                             const s64 AGroupId);
typedef s32(__stdcall *TGPGConfigureSignalReadWriteListDelete)(
    const s64 AModuleId, const s64 AActionId, const s32 AItemIndex);
typedef s32(__stdcall *TGPGConfigureModule)(const s64 AModuleId,
                                            const pchar AProgramName,
                                            const pchar ADisplayName,
                                            const s32 ARepeatCount,
                                            const bool ASelected);
typedef s32(__stdcall *TUIShowWindow)(const pchar AWindowTitle,
                                      const bool AIsShow);
typedef s32(__stdcall *TUIGraphicsLoadConfiguration)(
    const pchar AWindowTitle, const pchar AConfigurationName);
...
typedef s32(__stdcall *TTSAppSetSystemVarDoubleWTime)(const char *ACompleteName,
                                                      const double AValue,
                                                      const s64 ATimeUs);
typedef s32(__stdcall *TTSAppSetSystemVarInt32WTime)(const char *ACompleteName,
                                                     const s32 AValue,
                                                     const s64 ATimeUs);
typedef s32(__stdcall *TTSAppSetSystemVarUInt32WTime)(const char *ACompleteName,
                                                      const u32 AValue,
                                                      const s64 ATimeUs);
typedef s32(__stdcall *TTSAppSetSystemVarInt64WTime)(const char *ACompleteName,
                                                     const s64 AValue,
                                                     const s64 ATimeUs);
typedef s32(__stdcall *TTSAppSetSystemVarUInt64WTime)(const char *ACompleteName,
                                                      const u64 AValue,
                                                      const s64 ATimeUs);
typedef s32(__stdcall *TTSAppSetSystemVarUInt8ArrayWTime)(
    const char *ACompleteName, const s32 ACount, const pu8 AValue,
    const s64 ATimeUs);
typedef s32(__stdcall *TTSAppSetSystemVarInt32ArrayWTime)(
    const char *ACompleteName, const s32 ACount, const ps32 AValue,
    const s64 ATimeUs);
typedef s32(__stdcall *TTSAppSetSystemVarDoubleArrayWTime)(
    const char *ACompleteName, const s32 ACount, const double *AValue,
    const s64 ATimeUs);
typedef s32(__stdcall *TTSAppSetSystemVarStringWTime)(const char *ACompleteName,
                                                      const char *AValue,
                                                      const s64 ATimeUs);
typedef s32(__stdcall *TTSAppSetSystemVarGenericWTime)(
    const char *ACompleteName, const char *AValue, const s64 ATimeUs);
typedef s32(__stdcall *TTSAppSetSystemVarDoubleAsyncWTime)(
    const char *ACompleteName, const double AValue, const s64 ATimeUs);
typedef s32(__stdcall *TTSAppSetSystemVarInt32AsyncWTime)(
    const char *ACompleteName, const s32 AValue, const s64 ATimeUs);
typedef s32(__stdcall *TTSAppSetSystemVarUInt32AsyncWTime)(
    const char *ACompleteName, const u32 AValue, const s64 ATimeUs);
typedef s32(__stdcall *TTSAppSetSystemVarInt64AsyncWTime)(
    const char *ACompleteName, const s64 AValue, const s64 ATimeUs);
typedef s32(__stdcall *TTSAppSetSystemVarUInt64AsyncWTime)(
    const char *ACompleteName, const u64 AValue, const s64 ATimeUs);
typedef s32(__stdcall *TTSAppSetSystemVarUInt8ArrayAsyncWTime)(
    const char *ACompleteName, const s32 ACount, const pu8 AValue,
    const s64 ATimeUs);
typedef s32(__stdcall *TTSAppSetSystemVarInt32ArrayAsyncWTime)(
    const char *ACompleteName, const s32 ACount, const ps32 AValue,
    const s64 ATimeUs);
typedef s32(__stdcall *TTSAppSetSystemVarInt64ArrayAsyncWTime)(
    const char *ACompleteName, const s32 ACount, const ps64 AValue,
    const s64 ATimeUs);
typedef s32(__stdcall *TTSAppSetSystemVarDoubleArrayAsyncWTime)(
    const char *ACompleteName, const s32 ACount, const double *AValue,
    const s64 ATimeUs);
typedef s32(__stdcall *TTSAppSetSystemVarStringAsyncWTime)(
    const char *ACompleteName, const char *AValue, const s64 ATimeUs);
typedef s32(__stdcall *TTSAppSetSystemVarGenericAsyncWTime)(
    const char *ACompleteName, const char *AValue, const s64 ATimeUs);
typedef s32(__stdcall *TDBGetSignalStartBitByPDUOffset)(
    const s32 ASignalStartBitInPDU, const s32 ASignalBitLength,
    const bool AIsSignalIntel, const bool AIsPDUIntel, const s32 APDUStartBit,
    const s32 APDUBitLength, ps32 AActualStartBit);
typedef s32(__stdcall *TUIShowSaveFileDialog)(const char *ATitle,
                                              const char *AFileTypeDesc,
                                              const char *AFilter,
                                              const char *ASuggestFileName,
                                              char **ADestinationFileName);
typedef s32(__stdcall *TUIShowOpenFileDialog)(const char *ATitle,
                                              const char *AFileTypeDesc,
                                              const char *AFilter,
                                              const char *ASuggestFileName,
                                              char **ADestinationFileName);
typedef s32(__stdcall *TUIShowSelectDirectoryDialog)(
    char **ADestinationDirectory);
...
typedef s32(__stdcall *TRegisterSystemVarChangeEvent)(
    const void *AObj, const char *ACompleteName, const TOnSysVarChange AEvent);
typedef s32(__stdcall *TUnRegisterSystemVarChangeEvent)(
    const void *AObj, const char *ACompleteName, const TOnSysVarChange AEvent);
typedef s32(__stdcall *TUnRegisterSystemVarChangeEvents)(
    const void *AObj, const TOnSysVarChange AEvent);
typedef s32(__stdcall *TCallSystemAPI)(const char *AAPIName,
                                       const s32 AArgCount,
                                       const s32 AArgCapacity, char **AArgs);
typedef s32(__stdcall *TCallLibraryAPI)(const char *AAPIName,
                                        const s32 AArgCount,
                                        const s32 AArgCapacity, char **AArgs);
typedef s32(__stdcall *TIniReadStringWoQuotes)(
    const s32 AHandle, const char *ASection, const char *AKey,
    const char *AValue, ps32 AValueCapacity, const char *ADefault);
typedef s32(__stdcall *TUIGraphicsAddSignal)(const char *AWindowCaption,
                                             const s32 AIdxSplit,
                                             const TSignalType ASgnType,
                                             const char *ASignalAddress);
typedef s32(__stdcall *TUIGraphicsClearSignals)(const char *AWindowCaption,
                                                const s32 AIdxSplit);
typedef s32(__stdcall *TGPGLoadExcel)(const char *AFileName,
                                      char **AGraphicProgramName,
                                      char **ASubModuleName);
typedef s32(__stdcall *TRunProcedure)(const TCProcedure AProcedure);
typedef s32(__stdcall *TOpenHelpDoc)(const char *AFileNameWoSuffix,
                                     const char *ATitle);
typedef s32(__stdcall *TGetLangString)(const char *AEnglishStr,
                                       const char *AIniSection,
                                       char **ATranslatedStr);
typedef s32(__stdcall *TConvertBlfToCsv)(const char *ABlfFile,
                                         const char *ACSVFile,
                                         const pbool AToTerminate);
typedef s32(__stdcall *TConvertBlfToCsvWFilter)(const char *ABlfFile,
                                                const char *ACSVFile,
                                                const char *AFilterConf,
                                                const pbool AToTerminate);
typedef s32(__stdcall *TStartLogWFileName)(const void *AObj,
                                           const char *AFileName);
typedef s32(__stdcall *TConvertBlfToMatWFilter)(const char *ABlfFile,
                                                const char *AMatFile,
                                                const char *AFilterConf,
                                                const pbool AToTerminate);
typedef s32(__stdcall *TConvertASCToMatWFilter)(const char *AASCFile,
                                                const char *AMatFile,
                                                const char *AFilterConf,
                                                const pbool AToTerminate);
typedef s32(__stdcall *TConvertASCToCSVWFilter)(const char *AASCFile,
                                                const char *ACSVFile,
                                                const char *AFilterConf,
                                                const pbool AToTerminate);
typedef s32(__stdcall *TSetDebugLogLevel)(const TLogLevel ALevel);
typedef s32(__stdcall *TGetFormUniqueId)(const char *AClassName,
                                         const s32 AFormIdx, ps64 AUniqueId);
...
typedef struct _TTSApp {
  ...
  TAddDirectMappingWithFactorOffsetCAN
      add_direct_mapping_with_factor_offset_can;
  ...
  TDBGetFlexRayDBFramePropertiesByIndex
  ...
  TDBGetFlexRayDBSignalPropertiesByIndex
      db_get_flexray_signal_properties_by_index;
  ...
  TDBGetFlexRayDBPropertiesByAddress
      db_get_flexray_database_properties_by_address;
  ...
  TDBGetFlexRayDBECUPropertiesByAddress
      db_get_flexray_ecu_properties_by_address;
  ...
  TDBGetFlexRayDBFramePropertiesByAddress
      db_get_flexray_frame_properties_by_address;
  ...
  TDBGetFlexRayDBSignalPropertiesByAddress
      db_get_flexray_signal_properties_by_address;
  ...
  TDBGetFlexRayDBFramePropertiesByDBIndex
      db_get_flexray_frame_properties_by_db_index;
  ...
  TDBGetFlexRayDBSignalPropertiesByDBIndex
      db_get_flexray_signal_properties_by_db_index;
  TDBGetCANDBSignalPropertiesByFrameIndex
      db_get_can_signal_properties_by_frame_index;
  TDBGetLINDBSignalPropertiesByFrameIndex
      db_get_lin_signal_properties_by_frame_index;
  ...
  TGPGConfigureSignalReadWriteListClear
      gpg_configure_signal_read_write_list_clear;
  ...
  TGPGConfigureSignalReadWriteListDelete
  ...
  TTSAppSetSystemVarUInt8ArrayAsyncWTime
      set_system_var_uint8_array_async_w_time;
  TTSAppSetSystemVarInt32ArrayAsyncWTime
      set_system_var_int32_array_async_w_time;
  TTSAppSetSystemVarInt64ArrayAsyncWTime
      set_system_var_int64_array_async_w_time;
  ...
  s32 FDummy[709]; // >>> mp app end <<<
  s32 start_log_w_filename(const char *AFileName) {
    return internal_start_log_w_filename(FObj, AFileName);
  }
  s32 disconnect(void) { return internal_disconnect(FObj); }
  void terminate_application(void) { internal_terminate_application(FObj); }
  s32 wait(const s32 ATimeMs, const char *AMsg) {
    return internal_wait(FObj, ATimeMs, AMsg);
  }
  s32 check(const s32 AErrorCode) { return internal_check(FObj, AErrorCode); }
  s32 debug_log(const char *AFile, const char *AFunc, const s32 ALine,
                const char *AStr, const TLogLevel ALevel) {
    return internal_debug_log(FObj, AFile, AFunc, ALine, AStr, ALevel);
  }
  s32 start_log(void) { return internal_start_log(FObj); }
  s32 end_log(void) { return internal_end_log(FObj); }
  s32 write_realtime_comment(const char *AName) {
    return internal_write_realtime_comment(FObj, AName);
  }
  s32 check_terminate(void) { return internal_check_terminate(FObj); }
  ...
  s32 wait_with_dialog(const char *ATitle, const char *AMessage,
                       const bool *ApResult, const float *ApProgress100) {
    return internal_wait_with_dialog(FObj, ATitle, AMessage, ApResult,
                                     ApProgress100);
  }
  char *get_current_mp_name(void) { return internal_get_current_mp_name(FObj); }
  s32 register_system_var_change_event(const char *ACompleteName,
                                       const TOnSysVarChange AEvent) {
    return internal_register_system_var_change_event(FObj, ACompleteName,
                                                     AEvent);
  }
  s32 unregister_system_var_change_event(const char *ACompleteName,
                                         const TOnSysVarChange AEvent) {
    return internal_unregister_system_var_change_event(FObj, ACompleteName,
                                                       AEvent);
  }
...
typedef s32(__stdcall *TTransmitCANFDSync)(const PCANFD ACANFD,
                                           const s32 ATimeoutMS);
...
typedef s32(__stdcall *TTransmitFlexRaySync)(const PFlexRay AFlexRay,
                                             const s32 ATimeoutMS);
typedef double(__stdcall *TGetCANSignalValue)(const PCANSignal ACANSignal,
                                              const pu8 AData);
typedef void(__stdcall *TSetCANSignalValue)(const PCANSignal ACANSignal,
                                            const pu8 AData,
                                            const double AValue);
typedef double(__stdcall *TGetLINSignalValue)(const PLINSignal ACANSignal,
                                              const pu8 AData);
typedef void(__stdcall *TSetLINSignalValue)(const PLINSignal ACANSignal,
                                            const pu8 AData,
                                            const double AValue);
typedef double(__stdcall *TGetFlexRaySignalValue)(
    const PFlexRaySignal AFlexRaySignal, const pu8 AData);
typedef void(__stdcall *TSetFlexRaySignalValue)(
    const PFlexRaySignal AFlexRaySignal, const pu8 AData, const double AValue);
...
typedef void(__stdcall *TFlexRayEvent)(const ps32 AObj,
                                       const PFlexRay AFlexRay);
typedef void(__stdcall *TEthernetEvent)(const ps32 AObj,
                                        const PEthernetHeader AEthernet);
typedef s32(__stdcall *TRegisterCANEvent)(const ps32 AObj,
                                          const TCANEvent AEvent);
typedef s32(__stdcall *TUnregisterCANEvent)(const ps32 AObj,
                                            const TCANEvent AEvent);
typedef s32(__stdcall *TRegisterCANFDEvent)(const ps32 AObj,
                                            const TCANFDEvent AEvent);
typedef s32(__stdcall *TUnregisterCANFDEvent)(const ps32 AObj,
                                              const TCANFDEvent AEvent);
typedef s32(__stdcall *TRegisterLINEvent)(const ps32 AObj,
                                          const TLINEvent AEvent);
typedef s32(__stdcall *TUnregisterLINEvent)(const ps32 AObj,
                                            const TLINEvent AEvent);
typedef s32(__stdcall *TRegisterFlexRayEvent)(const ps32 AObj,
                                              const TFlexRayEvent AEvent);
typedef s32(__stdcall *TUnregisterFlexRayEvent)(const ps32 AObj,
                                                const TFlexRayEvent AEvent);
typedef s32(__stdcall *TRegisterEthernetEvent)(const ps32 AObj,
                                               const TEthernetEvent AEvent);
typedef s32(__stdcall *TUnregisterEthernetEvent)(const ps32 AObj,
                                                 const TEthernetEvent AEvent);
...
typedef s32(__stdcall *TRegisterPreTxCANEvent)(const ps32 AObj,
                                               const TCANEvent AEvent);
typedef s32(__stdcall *TUnregisterPreTxCANEvent)(const ps32 AObj,
                                                 const TCANEvent AEvent);
typedef s32(__stdcall *TRegisterPreTxCANFDEvent)(const ps32 AObj,
                                                 const TCANFDEvent AEvent);
typedef s32(__stdcall *TUnregisterPreTxCANFDEvent)(const ps32 AObj,
                                                   const TCANFDEvent AEvent);
typedef s32(__stdcall *TRegisterPreTxLINEvent)(const ps32 AObj,
                                               const TLINEvent AEvent);
typedef s32(__stdcall *TUnregisterPreTxLINEvent)(const ps32 AObj,
                                                 const TLINEvent AEvent);
typedef s32(__stdcall *TRegisterPreTxFlexRayEvent)(const ps32 AObj,
                                                   const TFlexRayEvent AEvent);
typedef s32(__stdcall *TUnregisterPreTxFlexRayEvent)(
    const ps32 AObj, const TFlexRayEvent AEvent);
typedef s32(__stdcall *TRegisterPreTxEthernetEvent)(
    const ps32 AObj, const TEthernetEvent AEvent);
typedef s32(__stdcall *TUnregisterPreTxEthernetEvent)(
    const ps32 AObj, const TEthernetEvent AEvent);
...
typedef s32(__stdcall *TGetBusStatistics)(
    const TLIBApplicationChannelType ABusType, const s32 AIdxChn,
    const TLIBCANBusStatistics AIdxStat, pdouble AStat);
typedef s32(__stdcall *TGetFPSCAN)(const s32 AIdxChn, const s32 AIdentifier,
                                   ps32 AFPS);
typedef s32(__stdcall *TGetFPSCANFD)(const s32 AIdxChn, const s32 AIdentifier,
                                     ps32 AFPS);
typedef s32(__stdcall *TGetFPSLIN)(const s32 AIdxChn, const s32 AIdentifier,
                                   ps32 AFPS);
typedef s32(__stdcall *TWaitCANMessage)(const void *AObj, const PCAN ATxCAN,
                                        const PCAN ARxCAN,
                                        const s32 ATimeoutMS);
typedef s32(__stdcall *TWaitCANFDMessage)(const void *AObj,
                                          const PCANFD ATxCANFD,
                                          const PCANFD ARxCANFD,
                                          const s32 ATimeoutMS);
typedef s32(__stdcall *TAddCyclicMsgCAN)(const PCAN ACAN,
                                         const float APeriodMS);
typedef s32(__stdcall *TAddCyclicMsgCANFD)(const PCANFD ACANFD,
                                           const float APeriodMS);
...
typedef s32(__stdcall *Tadd_precise_cyclic_message)(const s32 AIdentifier,
                                                    const u8 AChn,
                                                    const u8 AIsExt,
                                                    const float APeriodMS,
                                                    const s32 ATimeoutMS);
typedef s32(__stdcall *Tdelete_precise_cyclic_message)(const s32 AIdentifier,
                                                       const u8 AChn,
                                                       const u8 AIsExt,
                                                       const s32 ATimeoutMS);
...
typedef s32(__stdcall *TGetCANSignalDefinitionVerbose)(
    const s32 AIdxChn, const char *ANetworkName, const char *AMsgName,
    const char *ASignalName, ps32 AMsgIdentifier, PCANSignal ASignalDef);
typedef s32(__stdcall *TGetCANSignalDefinition)(const char *ASignalAddress,
                                                ps32 AMsgIdentifier,
                                                PCANSignal ASignalDef);
typedef s32(__stdcall *TGetFlexRaySignalDefinition)(const char *ASignalAddress,
                                                    PFlexRaySignal ASignalDef);
// online replay functions
typedef s32(__stdcall *Ttslog_add_online_replay_config)(const char *AFileName,
                                                        s32 *AIndex);
typedef s32(__stdcall *Ttslog_set_online_replay_config)(
    const s32 AIndex, const char *AName, const char *AFileName,
    const bool AAutoStart, const bool AIsRepetitiveMode,
    const TLIBOnlineReplayTimingMode AStartTimingMode,
    const s32 AStartDelayTimeMs, const bool ASendTx, const bool ASendRx,
    const char *AMappings);
typedef s32(__stdcall *Ttslog_get_online_replay_count)(s32 *ACount);
typedef s32(__stdcall *Ttslog_get_online_replay_config)(
    const s32 AIndex, char **AName, char **AFileName, bool *AAutoStart,
    bool *AIsRepetitiveMode, TLIBOnlineReplayTimingMode *AStartTimingMode,
    s32 *AStartDelayTimeMs, bool *ASendTx, bool *ASendRx, char **AMappings);
...
typedef s32(__stdcall *Ttslog_get_online_replay_status)(
    const s32 AIndex, TLIBOnlineReplayStatus *AStatus,
    float *AProgressPercent100);
// CAN rbs functions
...
typedef s32(__stdcall *TCANRBSConfigure)(
    const bool AAutoStart, const bool AAutoSendOnModification,
    const bool AActivateNodeSimulation,
    const TLIBRBSInitValueOptions AInitValueOptions);
typedef s32(__stdcall *TCANRBSEnable)(const bool AEnable);
typedef s32(__stdcall *TCANRBSActivateAllNetworks)(
    const bool AEnable, const bool AIncludingChildren);
typedef s32(__stdcall *TCANRBSActivateNetworkByName)(
    const s32 AIdxChn, const bool AEnable, const char *ANetworkName,
    const bool AIncludingChildren);
typedef s32(__stdcall *TCANRBSActivateNodeByName)(
    const s32 AIdxChn, const bool AEnable, const char *ANetworkName,
    const char *ANodeName, const bool AIncludingChildren);
typedef s32(__stdcall *TCANRBSActivateMessageByName)(const s32 AIdxChn,
                                                     const bool AEnable,
                                                     const char *ANetworkName,
                                                     const char *ANodeName,
                                                     const char *AMsgName);
typedef s32(__stdcall *TCANRBSSetMessageCycleByName)(const s32 AIdxChn,
                                                     const s32 AIntervalMs,
                                                     const char *ANetworkName,
                                                     const char *ANodeName,
                                                     const char *AMsgName);
typedef s32(__stdcall *TCANRBSGetSignalValueByElement)(
    const s32 AIdxChn, const char *ANetworkName, const char *ANodeName,
    const char *AMsgName, const char *ASignalName, double *AValue);
typedef s32(__stdcall *TCANRBSGetSignalValueByAddress)(
    const char *ASymbolAddress, double *AValue);
typedef s32(__stdcall *TCANRBSSetSignalValueByElement)(
    const s32 AIdxChn, const char *ANetworkName, const char *ANodeName,
    const char *AMsgName, const char *ASignalName, const double AValue);
typedef s32(__stdcall *TCANRBSSetSignalValueByAddress)(
    const char *ASymbolAddress, const double AValue);
typedef s32(__stdcall *TCANRBSBatchSetStart)(void);
typedef s32(__stdcall *TCANRBSBatchSetEnd)(void);
typedef s32(__stdcall *TCANRBSBatchSetSignal)(const char *ASymbolAddress,
                                              const double AValue);
typedef s32(__stdcall *TCANRBSSetMessageDirection)(const s32 AIdxChn,
                                                   const bool AIsTx,
                                                   const char *ANetworkName,
                                                   const char *ANodeName,
                                                   const char *AMsgName);
typedef s32(__stdcall *TCANRBSFaultInjectionClear)(void);
typedef s32(__stdcall *TCANRBSFaultInjectionMessageLost)(const bool AEnable,
                                                         const s32 AIdxChn,
                                                         const s32 AIdentifier);
typedef s32(__stdcall *TCANRBSFaultInjectionSignalAlter)(
    const bool AEnable, const char *ASymbolAddress, const double AAlterValue);
...
typedef s32(__stdcall *TCANRBSSetRCSignalWithLimit)(const char *ASymbolAddress,
                                                    const s32 ALowerLimit,
                                                    const s32 AUpperLimit);
typedef s32(__stdcall *TCANRBSSetCRCSignal)(const char *ASymbolAddress,
                                            const char *AAlgorithmName,
                                            const s32 AIdxByteStart,
                                            const s32 AByteCount);
...
typedef s32(__stdcall *TFlexRayRBSConfigure)(
    const bool AAutoStart, const bool AAutoSendOnModification,
    const bool AActivateECUSimulation,
    const TLIBRBSInitValueOptions AInitValueOptions);
typedef s32(__stdcall *TFlexRayRBSEnable)(const bool AEnable);
typedef s32(__stdcall *TFlexRayRBSActivateAllClusters)(
    const bool AEnable, const bool AIncludingChildren);
typedef s32(__stdcall *TFlexRayRBSActivateClusterByName)(
    const s32 AIdxChn, const bool AEnable, const char *AClusterName,
    const bool AIncludingChildren);
typedef s32(__stdcall *TFlexRayRBSActivateECUByName)(
    const s32 AIdxChn, const bool AEnable, const char *AClusterName,
    const char *AECUName, const bool AIncludingChildren);
typedef s32(__stdcall *TFlexRayRBSActivateFrameByName)(const s32 AIdxChn,
                                                       const bool AEnable,
                                                       const char *AClusterName,
                                                       const char *AECUName,
                                                       const char *AFrameName);
typedef s32(__stdcall *TFlexRayRBSGetSignalValueByElement)(
    const s32 AIdxChn, const char *AClusterName, const char *AECUName,
    const char *AFrameName, const char *ASignalName, double *AValue);
typedef s32(__stdcall *TFlexRayRBSGetSignalValueByAddress)(
    const char *ASymbolAddress, double *AValue);
typedef s32(__stdcall *TFlexRayRBSSetSignalValueByElement)(
    const s32 AIdxChn, const char *AClusterName, const char *AECUName,
    const char *AFrameName, const char *ASignalName, const double AValue);
typedef s32(__stdcall *TFlexRayRBSSetSignalValueByAddress)(
    const char *ASymbolAddress, const double AValue);
typedef s32(__stdcall *TFlexRayRBSBatchSetStart)(void);
typedef s32(__stdcall *TFlexRayRBSBatchSetEnd)(void);
typedef s32(__stdcall *TFlexRayRBSBatchSetSignal)(const char *ASymbolAddress,
                                                  const double AValue);
typedef s32(__stdcall *TFlexRayRBSSetFrameDirection)(const s32 AIdxChn,
                                                     const bool AIsTx,
                                                     const char *AClusterName,
                                                     const char *AECUName,
                                                     const char *AFrameName);
...
typedef s32(__stdcall *TFlexRayRBSSetRCSignalWithLimit)(
    const char *ASymbolAddress, const s32 ALowerLimit, const s32 AUpperLimit);
typedef s32(__stdcall *TFlexRayRBSSetCRCSignal)(const char *ASymbolAddress,
                                                const char *AAlgorithmName,
                                                const s32 AIdxByteStart,
                                                const s32 AByteCount);
// log file functions
typedef s32(__stdcall *Ttslog_blf_write_start)(const char *AFileName,
                                               s32 *AHandle);
typedef s32(__stdcall *Ttslog_blf_write_start_w_timestamp)(
    const char *AFileName, s32 *AHandle, u16 *AYear, u16 *AMonth, u16 *ADay,
    u16 *AHour, u16 *AMinute, u16 *ASecond, u16 *AMilliseconds);
typedef s32(__stdcall *Ttslog_blf_write_set_max_count)(const s32 AHandle,
                                                       const u32 ACount);
typedef s32(__stdcall *Ttslog_blf_write_can)(const s32 AHandle,
                                             const PCAN ACAN);
typedef s32(__stdcall *Ttslog_blf_write_can_fd)(const s32 AHandle,
                                                const PCANFD ACANFD);
typedef s32(__stdcall *Ttslog_blf_write_lin)(const s32 AHandle,
                                             const PLIN ALIN);
typedef s32(__stdcall *Ttslog_blf_write_flexray)(const s32 AHandle,
                                                 const PFlexRay AFlexRay);
typedef s32(__stdcall *Ttslog_blf_write_realtime_comment)(const s32 AHandle,
                                                          const s64 ATimeUs,
                                                          const char *AComment);
typedef s32(__stdcall *Ttslog_blf_write_end)(const s32 AHandle);
typedef s32(__stdcall *Ttslog_blf_read_start)(const char *AFileName,
                                              s32 *AHandle, s32 *AObjCount);
typedef s32(__stdcall *Ttslog_blf_read_status)(const s32 AHandle,
                                               s32 *AObjReadCount);
typedef s32(__stdcall *Ttslog_blf_read_object)(const s32 AHandle,
                                               s32 *AProgressedCnt,
                                               TSupportedBLFObjType *AType,
                                               PCAN ACAN, PLIN ALIN,
                                               PCANFD ACANFD);
typedef s32(__stdcall *Ttslog_blf_read_object_w_comment)(
    const s32 AHandle, s32 *AProgressedCnt, TSupportedBLFObjType *AType,
...
typedef s32(__stdcall *Ttslog_blf_seek_object_time)(const s32 AHandle,
                                                    const double AProg100,
                                                    s64 *ATime,
                                                    s32 *AProgressedCnt);
typedef s32(__stdcall *Ttslog_blf_to_asc)(
    const void *AObj, const char *ABLFFileName, const char *AASCFileName,
    const TProgressCallback AProgressCallback);
typedef s32(__stdcall *Ttslog_asc_to_blf)(
    const void *AObj, const char *AASCFileName, const char *ABLFFileName,
    const TProgressCallback AProgressCallback);
// IP functions
typedef s32(__stdcall *TIoIPCreate)(const void *AObj, const u16 APortTCP,
                                    const u16 APortUDP,
                                    const TOnIoIPData AOnTCPDataEvent,
                                    const TOnIoIPData AOnUDPDataEvent,
                                    s32 *AHandle);
typedef s32(__stdcall *TIoIPDelete)(const void *AObj, const s32 AHandle);
typedef s32(__stdcall *TIoIPEnableTCPServer)(const void *AObj,
                                             const s32 AHandle,
                                             const bool AEnable);
typedef s32(__stdcall *TIoIPEnableUDPServer)(const void *AObj,
                                             const s32 AHandle,
                                             const bool AEnable);
typedef s32(__stdcall *TIoIPConnectTCPServer)(const void *AObj,
                                              const s32 AHandle,
                                              const char *AIpAddress,
                                              const u16 APort);
typedef s32(__stdcall *TIoIPConnectUDPServer)(const void *AObj,
                                              const s32 AHandle,
                                              const char *AIpAddress,
                                              const u16 APort);
typedef s32(__stdcall *TIoIPDisconnectTCPServer)(const void *AObj,
                                                 const s32 AHandle);
typedef s32(__stdcall *TIoIPSendBufferTCP)(const void *AObj, const s32 AHandle,
                                           const pu8 APointer, const s32 ASize);
typedef s32(__stdcall *TIoIPSendBufferUDP)(const void *AObj, const s32 AHandle,
                                           const pu8 APointer, const s32 ASize);
typedef s32(__stdcall *TIoIPRecvTCPClientResponse)(const void *AObj,
                                                   const s32 AHandle,
                                                   const s32 ATimeoutMs,
                                                   const pu8 ABufferToReadTo,
                                                   ps32 AActualSize);
typedef s32(__stdcall *TIoIPSendTCPServerResponse)(const void *AObj,
                                                   const s32 AHandle,
                                                   const pu8 ABufferToWriteFrom,
                                                   const s32 ASize);
typedef s32(__stdcall *TIoIPSendUDPBroadcast)(const void *AObj,
                                              const s32 AHandle,
                                              const u16 APort,
                                              const pu8 ABufferToWriteFrom,
                                              const s32 ASize);
typedef s32(__stdcall *TIoIPSetUDPServerBufferSize)(const void *AObj,
                                                    const s32 AHandle,
                                                    const s32 ASize);
typedef s32(__stdcall *TIoIPRecvUDPClientResponse)(const void *AObj,
                                                   const s32 AHandle,
                                                   const s32 ATimeoutMs,
                                                   const pu8 ABufferToReadTo,
                                                   ps32 AActualSize);
typedef s32(__stdcall *TIoIPSendUDPServerResponse)(const void *AObj,
                                                   const s32 AHandle,
                                                   const pu8 ABufferToWriteFrom,
                                                   const s32 ASize);
// signal server
typedef s32(__stdcall *TSgnSrvRegisterCANSignalByMsgId)(const s32 AIdxChn,
                                                        const s32 AMsgId,
                                                        const char *ASgnName,
                                                        ps32 AClientId);
typedef s32(__stdcall *TSgnSrvRegisterLINSignalByMsgId)(const s32 AIdxChn,
                                                        const s32 AMsgId,
                                                        const char *ASgnName,
                                                        ps32 AClientId);
typedef s32(__stdcall *TSgnSrvRegisterCANSignalByMsgName)(
    const s32 AIdxChn, const char *ANetworkName, const char *AMsgName,
    const char *ASgnName, ps32 AClientId);
typedef s32(__stdcall *TSgnSrvRegisterLINSignalByMsgName)(
    const s32 AIdxChn, const char *ANetworkName, const char *AMsgName,
    const char *ASgnName, ps32 AClientId);
typedef s32(__stdcall *TSgnSrvGetCANSignalPhyValueLatest)(const s32 AIdxChn,
                                                          const s32 AClientId,
                                                          pdouble AValue,
                                                          ps64 ATimeUs);
typedef s32(__stdcall *TSgnSrvGetLINSignalPhyValueLatest)(const s32 AIdxChn,
                                                          const s32 AClientId,
                                                          pdouble AValue,
                                                          ps64 ATimeUs);
typedef s32(__stdcall *TSgnSrvGetCANSignalPhyValueInMsg)(const s32 AIdxChn,
                                                         const s32 AClientId,
                                                         const PCANFD AMsg,
                                                         pdouble AValue,
                                                         ps64 ATimeUs);
typedef s32(__stdcall *TSgnSrvGetLINSignalPhyValueInMsg)(const s32 AIdxChn,
                                                         const s32 AClientId,
                                                         const PLIN AMsg,
                                                         pdouble AValue,
                                                         ps64 ATimeUs);
typedef s32(__stdcall *TSgnSrvRegisterFlexRaySignalByFrame)(
    const s32 AIdxChn, const u8 AChnMask, const u8 ACycleNumber,
    const s32 ASlotId, const char *ASgnName, ps32 AClientId);
typedef s32(__stdcall *TSgnSrvRegisterFlexRaySignalByFrameName)(
    const s32 AIdxChn, const char *ANetworkName, const char *AFrameName,
    const char *ASgnName, ps32 AClientId);
typedef s32(__stdcall *TSgnSrvGetFlexRaySignalPhyValueLatest)(
    const s32 AIdxChn, const s32 AClientId, pdouble AValue, ps64 ATimeUs);
typedef s32(__stdcall *TSgnSrvGetFlexRaySignalPhyValueInFrame)(
    const s32 AIdxChn, const s32 AClientId, const PFlexRay AFrame,
    pdouble AValue, ps64 ATimeUs);
// pdu container
typedef s32(__stdcall *TPDUContainerSetCycleCount)(const s32 AIdxChn,
                                                   const s32 AMsgId,
                                                   const s32 ACount);
typedef s32(__stdcall *TPDUContainerSetCycleByIndex)(
    const s32 AIdxChn, const s32 AMsgId, const s32 AIdxCycle,
    const pchar ASignalGroupIdList);
typedef s32(__stdcall *TPDUContainerGetCycleCount)(const s32 AIdxChn,
                                                   const s32 AMsgId,
                                                   ps32 ACount);
typedef s32(__stdcall *TPDUContainerGetCycleByIndex)(const s32 AIdxChn,
                                                     const s32 AMsgId,
                                                     const s32 AIdxCycle,
                                                     char **ASignalGroupIdList);
typedef s32(__stdcall *TPDUContainerRefresh)(const s32 AIdxChn,
                                             const s32 AMsgId);
// j1939
typedef s32(__stdcall *TJ1939MakeId)(const s32 APGN, const u8 ASource,
                                     const u8 ADestination, const u8 APriority,
                                     ps32 AIdentifier);
typedef s32(__stdcall *TJ1939ExtractId)(const s32 AIdentifier, ps32 APGN,
                                        pu8 ASource, pu8 ADestination,
                                        pu8 APriority);
...
typedef s32(__stdcall *TJ1939GetDestination)(const s32 AIdentifier,
                                             pu8 ADestination);
...
typedef s32(__stdcall *TJ1939SetSource)(const ps32 AIdentifier,
                                        const u8 ASource);
typedef s32(__stdcall *TJ1939SetDestination)(const ps32 AIdentifier,
                                             const u8 ADestination);
typedef s32(__stdcall *TJ1939SetPriority)(const ps32 AIdentifier,
                                          const u8 APriority);
...
typedef s32(__stdcall *TJ1939GetLastPDU)(const u8 AIdxChn,
                                         const s32 AIdentifier,
                                         const bool AIsTx,
                                         const s32 APDUBufferSize,
                                         pu8 APDUBuffer, ps32 APDUActualSize,
                                         ps64 ATimeUs);
typedef s32(__stdcall *TJ1939GetLastPDUAsString)(
    const u8 AIdxChn, const s32 AIdentifier, const bool AIsTx, char **APDUData,
    ps32 APDUActualSize, ps64 ATimeUs);
typedef s32(__stdcall *TJ1939TransmitPDUAsync)(
    const u8 AIdxChn, const s32 APGN, const u8 APriority, const u8 ASource,
    const u8 ADestination, const pu8 APDUData, const s32 APDUSize);
typedef s32(__stdcall *TJ1939TransmitPDUSync)(
    const u8 AIdxChn, const s32 APGN, const u8 APriority, const u8 ASource,
    const u8 ADestination, const pu8 APDUData, const s32 APDUSize,
    const s32 ATimeoutMs);
typedef s32(__stdcall *TJ1939TransmitPDUAsStringAsync)(
    const u8 AIdxChn, const s32 APGN, const u8 APriority, const u8 ASource,
    const u8 ADestination, const char *APDUData);
typedef s32(__stdcall *TJ1939TransmitPDUAsStringSync)(
    const u8 AIdxChn, const s32 APGN, const u8 APriority, const u8 ASource,
    const u8 ADestination, const char *APDUData, const s32 ATimeoutMs);
typedef s32(__stdcall *TDisableOnlineReplayFilter)(const s32 AIndex);
typedef s32(__stdcall *TSetOnlineReplayFilter)(const s32 AIndex,
                                               const bool AIsPassFilter,
                                               const s32 ACount,
                                               const ps32 AIdxChannels,
                                               const ps32 AIdentifiers);
typedef s32(__stdcall *TSetCANSignalRawValue)(const PCANSignal ACANSignal,
                                              const pu8 AData,
                                              const u64 AValue);
typedef u64(__stdcall *TGetCANSignalRawValue)(const PCANSignal ACANSignal,
                                              const pu8 AData);
typedef s32(__stdcall *TSetLINSignalRawValue)(const PLINSignal ALINSignal,
                                              const pu8 AData,
                                              const u64 AValue);
typedef u64(__stdcall *TGetLINSignalRawValue)(const PLINSignal ALINSignal,
                                              const pu8 AData);
typedef s32(__stdcall *TSetFlexRaySignalRawValue)(
    const PFlexRaySignal AFlexRaySignal, const pu8 AData, const u64 AValue);
typedef u64(__stdcall *TGetFlexRaySignalRawValue)(
    const PFlexRaySignal AFlexRaySignal, const pu8 AData);
typedef s32(__stdcall *TFlexRayRBSUpdateFrameByHeader)(
    const TFlexRay *AFlexRay);
// LIN rbs functions
...
typedef s32(__stdcall *TLINRBSConfigure)(
    const bool AAutoStart, const bool AAutoSendOnModification,
    const bool AActivateNodeSimulation,
    const TLIBRBSInitValueOptions AInitValueOptions);
typedef s32(__stdcall *TLINRBSEnable)(const bool AEnable);
typedef s32(__stdcall *TLINRBSActivateAllNetworks)(
    const bool AEnable, const bool AIncludingChildren);
typedef s32(__stdcall *TLINRBSActivateNetworkByName)(
    const s32 AIdxChn, const bool AEnable, const char *ANetworkName,
    const bool AIncludingChildren);
typedef s32(__stdcall *TLINRBSActivateNodeByName)(
    const s32 AIdxChn, const bool AEnable, const char *ANetworkName,
    const char *ANodeName, const bool AIncludingChildren);
typedef s32(__stdcall *TLINRBSActivateMessageByName)(const s32 AIdxChn,
                                                     const bool AEnable,
                                                     const char *ANetworkName,
                                                     const char *ANodeName,
                                                     const char *AMsgName);
typedef s32(__stdcall *TLINBSSetMessageDelayTimeByName)(
    const s32 AIdxChn, const s32 AIntervalMs, const char *ANetworkName,
    const char *ANodeName, const char *AMsgName);
typedef s32(__stdcall *TLINRBSGetSignalValueByElement)(
    const s32 AIdxChn, const char *ANetworkName, const char *ANodeName,
    const char *AMsgName, const char *ASignalName, double *AValue);
typedef s32(__stdcall *TLINRBSGetSignalValueByAddress)(
    const char *ASymbolAddress, double *AValue);
typedef s32(__stdcall *TLINRBSSetSignalValueByElement)(
    const s32 AIdxChn, const char *ANetworkName, const char *ANodeName,
    const char *AMsgName, const char *ASignalName, const double AValue);
typedef s32(__stdcall *TLINRBSSetSignalValueByAddress)(
    const char *ASymbolAddress, const double AValue);
typedef s32(__stdcall *TLINRBSBatchSetStart)(void);
typedef s32(__stdcall *TLINRBSBatchSetEnd)(void);
typedef s32(__stdcall *TLINRBSBatchSetSignal)(const char *ASymbolAddress,
                                              const double AValue);
typedef s32(__stdcall *TTransmitEthernetASync)(
    const PEthernetHeader AEthernetHeader);
typedef s32(__stdcall *TTransmitEthernetSync)(
    const PEthernetHeader AEthernetHeader, const s32 ATimeoutMs);
typedef s32(__stdcall *TInjectEthernetFrame)(
    const PEthernetHeader AEthernetHeader);
typedef s32(__stdcall *TTSLogBlfWriteEthernet)(
    const s32 AHandle, const PEthernetHeader AEthernetHeader);

typedef s32(__stdcall *TTransmitEthernetAsyncWoPretx)(
    const PEthernetHeader AEthernetHeader);
typedef s32(__stdcall *TIoIpSetOnConnectionCallback)(
    const void *AObj, const s32 AHandle,
    const TOnIoIPConnection AConnectedCallback,
    const TOnIoIPConnection ADisconnectedCallback);
typedef s32(__stdcall *TEthBuildIPv4UDPPacket)(
    const PEthernetHeader AHeader, const pu8 ASrcIp, const pu8 ADstIp,
    const u16 ASrcPort, const u16 ADstPort, const pu8 APayload,
    const u16 APayloadLength, ps32 AIdentification, ps32 AFragmentIndex);
typedef s32(__stdcall *TBlockCurrentPreTx)(const void *AObj);
typedef s32(__stdcall *TEthernetIsUDPPacket)(const PEthernetHeader AHeader,
                                             pu16 AIdentification,
                                             pu16 AUDPPacketLength,
                                             pu16 AUDPDataOffset,
                                             pbool AIsPacketEnded);
typedef s32(__stdcall *TEthernetIPCalcHeaderChecksum)(
    const PEthernetHeader AHeader, const bool AOverwriteChecksum,
    pu16 AChecksum);
typedef s32(__stdcall *TEthernetUDPCalcChecksum)(const PEthernetHeader AHeader,
                                                 const pu8 AUDPPayloadAddr,
                                                 const u16 AUDPPayloadLength,
                                                 const bool AOverwriteChecksum,
                                                 pu16 AChecksum);
typedef s32(__stdcall *TEthernetUDPCalcChecksumOnFrame)(
    const PEthernetHeader AHeader, const bool AOverwriteChecksum,
    pu16 AChecksum);
...
typedef s32(__stdcall *Tlin_batch_add_schedule_frame)(const s32 AChnIdx,
                                                      const PLIN ALINData,
                                                      const s32 ADelayMs);
...
typedef s32(__stdcall *Tlin_set_node_functiontype)(const s32 AChnIdx,
                                                   const s32 AFunctionType);
typedef s32(__stdcall *Tlin_active_frame_in_schedule_table)(const u32 AChnIdx,
                                                            const u8 AID,
                                                            const s32 AIndex);
typedef s32(__stdcall *Tlin_deactive_frame_in_schedule_table)(const u32 AChnIdx,
                                                              const u8 AID,
                                                              const s32 AIndex);
typedef s32(__stdcall *Tflexray_disable_frame)(const s32 AChnIdx,
                                               const u8 ASlot,
                                               const u8 ABaseCycle,
                                               const u8 ACycleRep,
                                               const s32 ATimeoutMs);
typedef s32(__stdcall *Tflexray_enable_frame)(const s32 AChnIdx, const u8 ASlot,
                                              const u8 ABaseCycle,
                                              const u8 ACycleRep,
                                              const s32 ATimeoutMs);
typedef s32(__stdcall *Tflexray_start_net)(const s32 AChnIdx,
                                           const s32 ATimeoutMs);
typedef s32(__stdcall *Tflexray_stop_net)(const s32 AChnIdx,
                                          const s32 ATimeoutMs);
typedef s32(__stdcall *Tflexray_wakeup_pattern)(const s32 AChnIdx,
                                                const s32 ATimeoutMs);
...
typedef struct _TTSCOM {
  ...
  TSgnSrvRegisterFlexRaySignalByFrameName
      sgnsrv_register_flexray_signal_by_frame_name;
  TSgnSrvGetFlexRaySignalPhyValueLatest
      sgnsrv_get_flexray_signal_phy_value_latest;
  TSgnSrvGetFlexRaySignalPhyValueInFrame
      sgnsrv_get_flexray_signal_phy_value_in_frame;
  ...
  s32 FDummy[773]; // >>> mp com end <<<
  // internal functions
  s32 block_current_pretx() { return internal_block_current_pretx(FObj); }
  s32 wait_can_message(const PCAN ATxCAN, const PCAN ARxCAN,
                       const s32 ATimeoutMS) {
    return internal_wait_can_message(FObj, ATxCAN, ARxCAN, ATimeoutMS);
  }
  s32 wait_canfd_message(const PCANFD ATxCANFD, const PCANFD ARxCANFD,
                         const s32 ATimeoutMS) {
  ...
  s32 register_pretx_event_flexray(const ps32 AObj,
                                   const TFlexRayEvent AEvent) {
    return internal_register_pretx_event_flexray(AObj, AEvent);
  }
  s32 unregister_pretx_event_flexray(const ps32 AObj,
                                     const TFlexRayEvent AEvent) {
  ...
  s32 register_pretx_event_ethernet(const ps32 AObj,
                                    const TEthernetEvent AEvent) {
    return internal_register_pretx_event_ethernet(AObj, AEvent);
  }
  s32 unregister_pretx_event_ethernet(const ps32 AObj,
                                      const TEthernetEvent AEvent) {
  ...
  // IP functions
  s32 ioip_create(const u16 APortTCP, const u16 APortUDP,
                  const TOnIoIPData AOnTCPDataEvent,
                  const TOnIoIPData AOnUDPEvent, s32 *AHandle) {
    return internal_ioip_create(FObj, APortTCP, APortUDP, AOnTCPDataEvent,
                                AOnUDPEvent, AHandle);
  }
  ...
  s32 ioip_connect_tcp_server(const s32 AHandle, const char *AIpAddress,
                              const u16 APort) {
    return internal_ioip_connect_tcp_server(FObj, AHandle, AIpAddress, APort);
  }
  s32 ioip_connect_udp_server(const s32 AHandle, const char *AIpAddress,
                              const u16 APort) {
  ...
  s32 ioip_send_buffer_tcp(const s32 AHandle, const pu8 APointer,
                           const s32 ASize) {
    return internal_ioip_send_buffer_tcp(FObj, AHandle, APointer, ASize);
  }
  s32 ioip_send_buffer_udp(const s32 AHandle, const pu8 APointer,
                           const s32 ASize) {
    return internal_ioip_send_buffer_udp(FObj, AHandle, APointer, ASize);
  }
  s32 ioip_receive_tcp_client_response(const s32 AHandle, const s32 ATimeoutMs,
                                       const pu8 ABufferToReadTo,
                                       ps32 AActualSize) {
    return internal_ioip_receive_tcp_client_response(
        FObj, AHandle, ATimeoutMs, ABufferToReadTo, AActualSize);
  }
  s32 ioip_send_tcp_server_response(const s32 AHandle,
                                    const pu8 ABufferToWriteFrom,
                                    const s32 ASize) {
    return internal_ioip_send_tcp_server_response(FObj, AHandle,
                                                  ABufferToWriteFrom, ASize);
  }
  s32 ioip_send_udp_broadcast(const s32 AHandle, const u16 APort,
                              const pu8 ABufferToWriteFrom, const s32 ASize) {
    return internal_ioip_send_udp_broadcast(FObj, AHandle, APort,
                                            ABufferToWriteFrom, ASize);
  }
  ...
  s32 ioip_receive_udp_client_response(const s32 AHandle, const s32 ATimeoutMs,
                                       const pu8 ABufferToReadTo,
                                       ps32 AActualSize) {
    return internal_ioip_receive_udp_client_response(
        FObj, AHandle, ATimeoutMs, ABufferToReadTo, AActualSize);
  }
  s32 ioip_send_udp_server_response(const s32 AHandle,
                                    const pu8 ABufferToWriteFrom,
                                    const s32 ASize) {
    return internal_ioip_send_udp_server_response(FObj, AHandle,
                                                  ABufferToWriteFrom, ASize);
  }
  s32 ioip_set_tcp_server_connection_callback(
      const s32 AHandle, const TOnIoIPConnection AConnectedCallback,
      const TOnIoIPConnection ADisconnectedCallback) {
    return internal_ioip_set_tcp_server_connection_callback(
        FObj, AHandle, AConnectedCallback, ADisconnectedCallback);
  }
} TTSCOM, *PTSCOM;
...
typedef s32(__stdcall *TTestCheckVerdict)(const void *AObj, const char *AName,
                                          const double AValue,
                                          const double AMin, const double AMax);
typedef s32(__stdcall *TTestLog)(const void *AObj, const char *AStr,
                                 const TLogLevel ALevel);
typedef s32(__stdcall *TTestLogValue)(const void *AObj, const char *AStr,
                                      const double AValue,
                                      const TLogLevel ALevel);
typedef s32(__stdcall *TTestLogString)(const void *AObj, const char *AStr,
                                       const char *AInfo,
                                       const TLogLevel ALevel);
typedef s32(__stdcall *TTestDebugLog)(const void *AObj, const char *AFile,
                                      const char *AFunc, const s32 ALine,
                                      const char *AStr, const TLogLevel ALevel);
typedef s32(__stdcall *TTestWriteResultString)(const void *AObj,
                                               const char *AName,
                                               const char *AValue,
                                               const TLogLevel ALevel);
typedef s32(__stdcall *TTestWriteResultValue)(const void *AObj,
                                              const char *AName,
                                              const double AValue,
                                              const TLogLevel ALevel);
typedef s32(__stdcall *TTestCheckErrorBegin)(void);
typedef s32(__stdcall *TTestCheckErrorEnd)(const ps32 AErrorCount);
typedef s32(__stdcall *TTestWriteResultImage)(const void *AObj,
                                              const char *AName,
                                              const char *AImageFileFullPath);
typedef s32(__stdcall *TTestRetrieveCurrentResultFolder)(const void *AObj,
                                                         char **AFolder);
typedef s32(__stdcall *TTestCheckTerminate)(void);
typedef s32(__stdcall *TTestSignalCheckerClear)(void);
typedef s32(__stdcall *TTestSignalCheckerAddCheckWithTime)(
    const TSignalType ASgnType, const TSignalCheckKind ACheckKind,
    const char *ASgnName, const double ASgnMin, const double ASgnMax,
    const double ATimeStartS, const double ATimeEndS, ps32 ACheckId);
typedef s32(__stdcall *TTestSignalCheckerAddCheckWithTrigger)(
    const TSignalType ASgnType, const TSignalCheckKind ACheckKind,
    const char *ASgnName, const double ASgnMin, const double ASgnMax,
    const TSignalType ATriggerType, const char *ATriggerName,
    const double ATriggerMin, const double ATriggerMax, ps32 ACheckId);
typedef s32(__stdcall *TTestSignalCheckerAddStatisticsWithTime)(
    const TSignalType ASgnType, const TSignalStatisticsKind AStatisticsKind,
    const char *ASgnName, const double ATimeStartS, const double ATimeEndS,
    ps32 ACheckId);
typedef s32(__stdcall *TTestSignalCheckerAddStatisticsWithTrigger)(
    const TSignalType ASgnType, const TSignalStatisticsKind AStatisticsKind,
    const char *ASgnName, const TSignalType ATriggerType,
    const char *ATriggerName, const double ATriggerMin,
    const double ATriggerMax, ps32 ACheckId);
typedef s32(__stdcall *TTestSignalCheckerGetResult)(const void *AObj,
                                                    const s32 ACheckId,
                                                    bool *APass,
                                                    pdouble AResult,
                                                    char **AResultRepr);
typedef s32(__stdcall *TTestSignalCheckerEnable)(const s32 ACheckId,
                                                 const bool AEnable);
typedef s32(__stdcall *TTestSignalCheckerAddRisingEdgeWithTime)(
    const TSignalType ASgnType, const char *ASgnName, const double ATimeStartS,
    const double ATimeEndS, ps32 ACheckId);
typedef s32(__stdcall *TTestSignalCheckerAddRisingEdgeWithTrigger)(
    const TSignalType ASgnType, const char *ASgnName,
    const TSignalType ATriggerType, const char *ATriggerName,
    const double ATriggerMin, const double ATriggerMax, ps32 ACheckId);
typedef s32(__stdcall *TTestSignalCheckerAddFallingEdgeWithTime)(
    const TSignalType ASgnType, const char *ASgnName, const double ATimeStartS,
    const double ATimeEndS, ps32 ACheckId);
typedef s32(__stdcall *TTestSignalCheckerAddFallingEdgeWithTrigger)(
    const TSignalType ASgnType, const char *ASgnName,
    const TSignalType ATriggerType, const char *ATriggerName,
    const double ATriggerMin, const double ATriggerMax, ps32 ACheckId);
typedef s32(__stdcall *TTestSignalCheckerAddMonotonyRisingWithTime)(
    const TSignalType ASgnType, const char *ASgnName,
    const s32 ASampleIntervalMs, const double ATimeStartS,
    const double ATimeEndS, ps32 ACheckId);
typedef s32(__stdcall *TTestSignalCheckerAddMonotonyRisingWithTrigger)(
    const TSignalType ASgnType, const char *ASgnName,
    const s32 ASampleIntervalMs, const TSignalType ATriggerType,
    const char *ATriggerName, const double ATriggerMin,
    const double ATriggerMax, ps32 ACheckId);
typedef s32(__stdcall *TTestSignalCheckerAddMonotonyFallingWithTime)(
    const TSignalType ASgnType, const char *ASgnName,
    const s32 ASampleIntervalMs, const double ATimeStartS,
    const double ATimeEndS, ps32 ACheckId);
typedef s32(__stdcall *TTestSignalCheckerAddMonotonyFallingWithTrigger)(
    const TSignalType ASgnType, const char *ASgnName,
    const s32 ASampleIntervalMs, const TSignalType ATriggerType,
    const char *ATriggerName, const double ATriggerMin,
    const double ATriggerMax, ps32 ACheckId);
typedef s32(__stdcall *TTestSignalCheckerAddFollowWithTime)(
    const TSignalType ASgnType, const TSignalType AFollowSignalType,
    const char *ASgnName, const char *AFollowSgnName, const double AErrorRange,
    const double ATimeStartS, const double ATimeEndS, ps32 ACheckId);
typedef s32(__stdcall *TTestSignalCheckerAddFollowWithTrigger)(
    const TSignalType ASgnType, const TSignalType AFollowSignalType,
    const char *ASgnName, const char *AFollowSgnName, const double AErrorRange,
    const TSignalType ATriggerType, const char *ATriggerName,
    const double ATriggerMin, const double ATriggerMax, ps32 ACheckId);
typedef s32(__stdcall *TTestSignalCheckerAddJumpWithTime)(
    const TSignalType ASgnType, const char *ASgnName, const bool AIgnoreFrom,
    const double AFrom, const double ATo, const double ATimeStartS,
    const double ATimeEndS, ps32 ACheckId);
typedef s32(__stdcall *TTestSignalCheckerAddJumpWithTrigger)(
    const TSignalType ASgnType, const char *ASgnName, const bool AIgnoreFrom,
    const double AFrom, const double ATo, const TSignalType ATriggerType,
    const char *ATriggerName, const double ATriggerMin,
    const double ATriggerMax, ps32 ACheckId);
typedef s32(__stdcall *TTestSignalCheckerAddUnChangeWithTime)(
    const TSignalType ASgnType, const char *ASgnName, const double ATimeStartS,
    const double ATimeEndS, ps32 ACheckId);
typedef s32(__stdcall *TTestSignalCheckerAddUnChangeWithTrigger)(
    const TSignalType ASgnType, const char *ASgnName,
    const TSignalType ATriggerType, const char *ATriggerName,
    const double ATriggerMin, const double ATriggerMax, ps32 ACheckId);
typedef s32(__stdcall *TTestSignalCheckerCheckStatistics)(
    const void *AObj, const s32 ACheckId, const double AMin, const double AMax,
    bool *APass, pdouble AResult, char **AResultRepr);
...
typedef s32(__stdcall *TSignalTesterGetItemVerdictByIndex)(const void *AObj,
                                                           const s32 AIndex,
                                                           pbool AIsPass);
typedef s32(__stdcall *TSignalTesterGetItemResultByName)(
    const void *AObj, const char *AName, pbool AIsPass, ps64 AEventTimeUs,
    char **AValues, char **ADescription);
typedef s32(__stdcall *TSignalTesterGetItemResultByIndex)(
    const void *AObj, const s32 AIndex, pbool AIsPass, ps64 AEventTimeUs,
    char **AValues, char **ADescription);
typedef s32(__stdcall *TSignalTesterGetItemVerdictByName)(const void *AObj,
                                                          const char *AName,
                                                          pbool AIsPass);
typedef s32(__stdcall *TSignalTesterCheckStatisticsByIndex)(
    const void *AObj, const s32 AIndex, const double AMin, const double AMax,
    bool *APass, char **AValues, char **AResultRepr);
typedef s32(__stdcall *TSignalTesterCheckStatisticsByName)(
    const void *AObj, const char *AItemName, const double AMin,
    const double AMax, bool *APass, char **AValues, char **AResultRepr);
typedef s32(__stdcall *TSignalTesterEnableItem)(const s32 AIndex,
                                                const bool AEnable);
typedef s32(__stdcall *TSignalTesterEnableItemByName)(const char *AItemName,
                                                      const bool AEnable);
...
typedef s32(__stdcall *TSignalTesterGetItemStatusByIndex)(
    const s32 AIdx, pbool AIsRunning, pbool AIsCheckDone,
    PSignalTesterFailReason AFailReason);
typedef s32(__stdcall *TSignalTesterGetItemStatusByName)(
    const char *ATesterName, pbool AIsRunning, pbool AIsCheckDone,
    PSignalTesterFailReason AFailReason);
typedef s32(__stdcall *TSignalTesterSetItemTimeRangeByIndex)(
    const s32 AIdx, const double ATimeBegin, const double ATimeEnd);
typedef s32(__stdcall *TSignalTesterSetItemTimeRangeByName)(
    const char *AName, const double ATimeBegin, const double ATimeEnd);
typedef s32(__stdcall *TSignalTesterSetItemTimeRangeByIndex)(
    const s32 AIdx, const double ALow, const double AHigh);
typedef s32(__stdcall *TSignalTesterSetItemTimeRangeByName)(const char *AName,
                                                            const double ALow,
                                                            const double AHigh);
...
typedef struct _TTSTest {
  ...
  TTestSignalCheckerAddStatisticsWithTime
      signal_checker_add_statistics_with_time;
  TTestSignalCheckerAddStatisticsWithTrigger
      signal_checker_add_statistics_with_trigger;
  ...
  TTestSignalCheckerAddRisingEdgeWithTime
      signal_checker_add_rising_edge_with_time;
  TTestSignalCheckerAddRisingEdgeWithTrigger
      signal_checker_add_rising_edge_with_trigger;
  TTestSignalCheckerAddFallingEdgeWithTime
      signal_checker_add_falling_edge_with_time;
  TTestSignalCheckerAddFallingEdgeWithTrigger
      signal_checker_add_falling_edge_with_trigger;
  TTestSignalCheckerAddMonotonyRisingWithTime
      signal_checker_add_monotony_rising_with_time;
  TTestSignalCheckerAddMonotonyRisingWithTrigger
      signal_checker_add_monotony_rising_with_trigger;
  TTestSignalCheckerAddMonotonyFallingWithTime
      signal_checker_add_monotony_falling_with_time;
  TTestSignalCheckerAddMonotonyFallingWithTrigger
      signal_checker_add_monotony_falling_with_trigger;
  ...
  TTestSignalCheckerAddUnChangeWithTrigger
      signal_checker_add_unchange_with_trigger;
  ...
  TSignalTesterGetItemVerdictByIndex
      internal_signal_tester_get_item_verdict_by_index;
  TSignalTesterGetItemResultByName
      internal_signal_tester_get_item_result_by_name;
  TSignalTesterGetItemResultByIndex
      internal_signal_tester_get_item_result_by_index;
  TSignalTesterGetItemVerdictByName
      internal_signal_tester_get_item_verdict_by_name;
  TSignalTesterCheckStatisticsByIndex
      internal_signal_tester_check_statistics_by_index;
  TSignalTesterCheckStatisticsByName
      internal_signal_tester_check_statistics_by_name;
  ...
  TSignalTesterSetItemTimeRangeByIndex
      signal_tester_set_item_time_range_by_index;
  TSignalTesterSetItemTimeRangeByName signal_tester_set_item_time_range_by_name;
  TSignalTesterSetItemTimeRangeByIndex
      signal_tester_set_item_value_range_by_index;
  TSignalTesterSetItemTimeRangeByName
      signal_tester_set_item_value_range_by_name;
  s32 FDummy[945]; // >>> mp test end <<<
  void set_verdict_ok(const char *AStr) { internal_set_verdict_ok(FObj, AStr); }
  ...
  s32 check_verdict(const char *AName, const double AValue, const double AMin,
                    const double AMax) {
  ...
  void log_value(const char *AStr, const double AValue,
                 const TLogLevel ALevel) {
    internal_log_value(FObj, AStr, AValue, ALevel);
  }
  void log_string(const char *AStr, const char *AValue,
                  const TLogLevel ALevel) {
    internal_log_string(FObj, AStr, AValue, ALevel);
  }
  void debug_log_info(const char *AFile, const char *AFunc, const s32 ALine,
                      const char *AStr, const TLogLevel ALevel) {
    internal_debug_log_info(FObj, AFile, AFunc, ALine, AStr, ALevel);
  }
  void write_result_string(const char *AName, const char *AValue,
                           const TLogLevel ALevel) {
    internal_write_result_string(FObj, AName, AValue, ALevel);
  }
  void write_result_value(const char *AName, const double AValue,
                          const TLogLevel ALevel) {
  ...
  s32 signal_checker_get_result(const s32 ACheckId, bool *APass,
                                pdouble AResult, char **AResultRepr) {
    return internal_signal_checker_get_result(FObj, ACheckId, APass, AResult,
                                              AResultRepr);
  }
  s32 signal_checker_check_statistics(const s32 ACheckId, const double AMin,
                                      const double AMax, bool *APass,
                                      pdouble AResult, char **AResultRepr) {
    return internal_signal_checker_check_statistics(
        FObj, ACheckId, AMin, AMax, APass, AResult, AResultRepr);
  }
  s32 signal_tester_get_item_verdict_by_index(const s32 AIndex, pbool AIsPass) {
    return internal_signal_tester_get_item_verdict_by_index(FObj, AIndex,
                                                            AIsPass);
  }
  s32 signal_tester_get_item_result_by_name(const char *AName, pbool AIsPass,
                                            ps64 AEventTimeUs, char **AValues,
                                            char **ADescription) {
    return internal_signal_tester_get_item_result_by_name(
        FObj, AName, AIsPass, AEventTimeUs, AValues, ADescription);
  }
  s32 signal_tester_get_item_result_by_index(const s32 AIndex, pbool AIsPass,
                                             ps64 AEventTimeUs, char **AValues,
                                             char **ADescription) {
    return internal_signal_tester_get_item_result_by_index(
        FObj, AIndex, AIsPass, AEventTimeUs, AValues, ADescription);
  }
  s32 signal_tester_get_item_verdict_by_name(const char *AName, pbool AIsPass) {
    return internal_signal_tester_get_item_verdict_by_name(FObj, AName,
                                                           AIsPass);
  }
  s32 signal_tester_check_statistics_by_index(const s32 AIndex,
                                              const double AMin,
                                              const double AMax, bool *APass,
                                              char **AResults,
                                              char **AResultRepr) {
    return internal_signal_tester_check_statistics_by_index(
        FObj, AIndex, AMin, AMax, APass, AResults, AResultRepr);
  }
  s32 signal_tester_check_statistics_by_name(const char *AItemName,
                                             const double AMin,
                                             const double AMax, bool *APass,
                                             char **AResults,
                                             char **AResultRepr) {
    return internal_signal_tester_check_statistics_by_name(
        FObj, AItemName, AMin, AMax, APass, AResults, AResultRepr);
  }
} TTSTest, *PTSTest;
...
extern void internal_log(const char *AFile, const char *AFunc, const s32 ALine,
                         const TLogLevel ALevel, const char *fmt, ...);
extern void internal_test_log(const char *AFile, const char *AFunc,
...
#undef log
#define log(...)                                                               \
  internal_log(__FILE__, __FUNCTION__, __LINE__, lvlInfo, __VA_ARGS__)
#define printf(...)                                                            \
  internal_log(__FILE__, __FUNCTION__, __LINE__, lvlInfo, __VA_ARGS__)
#define log_ok(...)                                                            \
  internal_log(__FILE__, __FUNCTION__, __LINE__, lvlOK, __VA_ARGS__)
#define log_nok(...)                                                           \
  internal_log(__FILE__, __FUNCTION__, __LINE__, lvlError, __VA_ARGS__)
#define log_hint(...)                                                          \
  internal_log(__FILE__, __FUNCTION__, __LINE__, lvlHint, __VA_ARGS__)
#define log_warning(...)                                                       \
  internal_log(__FILE__, __FUNCTION__, __LINE__, lvlWarning, __VA_ARGS__)
#define test_log(...)                                                          \
  internal_test_log(__FILE__, __FUNCTION__, __LINE__, lvlInfo, __VA_ARGS__)
#define test_log_ok(...)                                                       \
  internal_test_log(__FILE__, __FUNCTION__, __LINE__, lvlOK, __VA_ARGS__)
#define test_log_nok(...)                                                      \
  internal_test_log(__FILE__, __FUNCTION__, __LINE__, lvlError, __VA_ARGS__)
...
#define IDX_ERR_HID_SEND_FEATURE_RPT                                           \
...
#define IDX_ERR_HW_DFU_COMMAND_TIMED_OUT                                       \
...
#define IDX_ERR_SW_API_REPLAY_ON_GOING                                         \
...
#define IDX_ERR_INVALID_CHANNEL_MAPPING                                        \
  81 /* Invalid application channel mapping */
#define IDX_ERR_TSLIB_GENERIC_OPERATION_FAILED                                 \
...
#define IDX_ERR_APPNAME_LENGTH_OUT_OF_RANGE                                    \
...
#define IDX_ERR_LIB_NOT_INITIALIZED                                            \
  97 /* cannot find libTSMaster.dll, use "set_libtsmaster_location" to set its \
        location before calling initialize_lib_tsmaster */
...
#define IDX_ERR_SIGNAL_MULTIPLEXED_NOT_ACTIVE                                  \
  112 /* Current multiplexed signal is not active */
#define IDX_ERR_GET_HANDLE_BY_CHANNEL_FAILED                                   \
  113 /* Get handle by logic channel failed */
#define IDX_ERR_CANNOT_OPERATE_WHILE_APP_CONN                                  \
  114 /* Cannot operate while application is connected, please stop            \
         application first */
...
#define IDX_ERR_CURRENT_LIB_NOT_SUPPORTED                                      \
...
#define IDX_ERR_READ_DATA_PACKAGE_OVERFLOW                                     \
...
#define IDX_ERR_WAIT_CRITERIA_NOT_SATISFIED                                    \
  136 /* Wait criteria not satisfied */
#define IDX_ERR_REQUIRE_APP_CONNECTED                                          \
  137 /* Operation requires application connected */
#define IDX_ERR_PROJECT_PATH_ALREADY_USED                                      \
  138 /* Project path is used by another application */
#define IDX_ERR_TP_TIMEOUT_AS                                                  \
  139 /* Timeout for the sender to transmit data to the receiver */
#define IDX_ERR_TP_TIMEOUT_AR                                                  \
  140 /* Timeout for the receiver to transmit flow control to the sender */
#define IDX_ERR_TP_TIMEOUT_BS                                                  \
  141 /* Timeout for the sender to send first data frame after receiving FC    \
         frame */
#define IDX_ERR_TP_TIMEOUT_CR                                                  \
  142 /* Timeout for the receiver to receiving first CF frame after sending FC \
         frame */
#define IDX_ERR_TP_WRONG_SN 143 /* Serial Number Error */
#define IDX_ERR_TP_INVALID_FS                                                  \
  144 /* Invalid flow status of the flow control frame */
#define IDX_ERR_TP_UNEXP_PDU 145 /* Unexpected Protocol Data Unit */
#define IDX_ERR_TP_WFT_OVRN                                                    \
...
#define IDX_ERR_UDS_MODULE_NOT_EXISTING                                        \
...
#define IDX_ERR_UDS_TIMEOUT_SENDING_REQUEST                                    \
  160                                        /* Timeout to send uds request */
...
#define IDX_ERR_UDS_NEGATIVE_WITH_EXPECTED_NRC                                 \
  163 /* Get uds negative response with expected NRC */
#define IDX_ERR_UDS_NEGATIVE_UNEXPECTED_NRC                                    \
...
#define IDX_ERR_UDS_UNEXPECTED_POSTIVE_RESPONSE                                \
  168 /* Receive unpexted positive response frame */
#define IDX_ERR_UDS_POSITIVE_REPONSE_WITH_WRONG_DATA                           \
  169 /* Receive positive response with wrong data */
#define IDX_ERR_UDS_GET_POSITIVE_RESPONSE_FAILED                               \
  170 /* Failed to get positive response */
#define IDX_ERR_UDS_MaxNumOfBlockLen_OVER_FLOW                                 \
  171 /* MaxNumOfBlockLen out of range */
#define IDX_ERR_UDS_NEGATIVE_RESPONSE_WITH_UNEXPECTED_NRC                      \
  172 /* Receive negative response with unexpected NRC */
#define IDX_ERR_UDS_SERVICE_IS_RUNNING 173 /* UDS serive is busy */
#define IDX_ERR_UDS_NEED_APPLY_DOWNLOAD_FIRST                                  \
  174 /* Apply download first before transfer data */
#define IDX_ERR_UDS_RESPONSE_DATA_LENGTH_ERR                                   \
  175 /* Length of the uds reponse is wrong */
#define IDX_ERR_TEST_CHECK_LOWER                                               \
  176 /* Verdict value smaller than specification */
#define IDX_ERR_TEST_CHECK_UPPER                                               \
  177 /* Verdict value greater than specification */
#define IDX_ERR_TEST_VERDICT_CHECK_FAILED 178 /* Verdict check failed */
#define IDX_ERR_AM_NOT_LOADED                                                  \
  179 /* Graphic Program not loaded, please load it first */
#define IDX_ERR_PANEL_NOT_FOUND 180 /* Panel not found */
#define IDX_ERR_CONTROL_NOT_FOUND_IN_PANEL                                     \
  181 /* Control not found in panel                                            \
       */
#define IDX_ERR_PANEL_NOT_LOADED                                               \
  182 /* Panel not loaded, please load it first */
#define IDX_ERR_STIM_SIGNAL_NOT_FOUND 183 /* STIM signal not found */
#define IDX_ERR_AM_SUB_MODULE_NOT_AVAIL                                        \
  184 /* Graphic Program sub module not available */
#define IDX_ERR_AM_VARIANT_GROUP_NOT_FOUND                                     \
  185 /* Graphic Program variant group not found */
#define IDX_ERR_PANEL_CONTROL_NOT_FOUND 186 /* Control not found in panel */
#define IDX_ERR_PANEL_CONTROL_NOT_SUPPORT_THIS                                 \
  187 /* Panel control does not support this property */
#define IDX_ERR_RBS_NOT_RUNNING 188 /* RBS engine is not running */
#define IDX_ERR_MSG_NOT_SUPPORT_PDU_CONTAINER                                  \
...
#define IDX_ERR_J1939_ANOTHER_PDU_IS_SENDING                                   \
  192 /* Transmit J1939 PDU failed due to another PDU is already being         \
         transmitted */
#define IDX_ERR_J1939_TX_FAILED_PROTOCOL_ERROR                                 \
  193 /* Transmit J1939 PDU failed due to protocol error */
#define IDX_ERR_J1939_TX_FAILED_NODE_INACTIVE                                  \
  194 /* Transmit J1939 PDU failed due to node inactive */
#define IDX_ERR_NO_LICENSE 195 /* API is called without license support*/
#define IDX_ERR_SIGNAL_CHECK_RANGE_VIOLATION                                   \
  196 /* Signal range check violation */
#define IDX_ERR_LOG_READ_CATEGORY_FAILED                                       \
  197 /* DataLogger read category failed */
#define IDX_ERR_CHECK_BOOT_VERSION_FAILED                                      \
  198 /* Check Flash Bootloader Version Failed */
#define IDX_ERR_LOG_FILE_NOT_CREATED 199 /* Log file not created */
#define IDX_ERR_MODULE_IS_BEING_EDITED_BY_USER                                 \
  200 /* the current module is being edited by user in dialog */
#define IDX_ERR_LOG_DEVICE_IS_BUSY                                             \
  201 /* The Logger device is busy, can not operation at the same time */
#define IDX_ERR_LIN_MASTER_TRANSMIT_N_AS_TIMEOUT                               \
  202 /* Master node transmit diagnostic package timeout */
#define IDX_ERR_LIN_MASTER_TRANSMIT_TRANSMIT_ERROR                             \
  203 /* Master node transmit frame failed */
#define IDX_ERR_LIN_MASTER_REV_N_CR_TIMEOUT                                    \
  204 /* Master node receive diagnostic package timeout */
#define IDX_ERR_LIN_MASTER_REV_ERROR                                           \
  205 /* Master node receive frame failed                                      \
       */
#define IDX_ERR_LIN_MASTER_REV_INTERLLEAVE_TIMEOUT                             \
  206 /* Internal time runs out before reception is completed */
#define IDX_ERR_LIN_MASTER_REV_NO_RESPONSE                                     \
  207 /* Master node received no response */
#define IDX_ERR_LIN_MASTER_REV_SN_ERROR                                        \
...
#define IDX_ERR_LIN_SLAVE_REV_N_CR_TIMEOUT                                     \
...
#define IDX_ERR_HALTED_DUE_TO_USER_BREAK                                       \
  216                                 /* Operation halted due to user break */
#define IDX_ERR_WRITE_FILE_FAILED 217 /* Write file failed */
#define IDX_ERR_UNKNOWN_OBJECT_DETECTED 218 /* Unknown object detected */
#define IDX_ERR_THIS_FUNC_SHOULD_BE_CALLED_IN_MP                               \
  219 /* this function should be called in mini program thread */
#define IDX_ERR_USER_CANCEL_WAIT 220       /* user canceled wait */
#define IDX_ERR_DECOMPRESS_DATA_FAILED 221 /* Decompress data failed */
#define IDX_ERR_AUTOMATION_OBJ_NOT_CREATED                                     \
  222                               /* Automation object not created */
#define IDX_ERR_ITEM_DUPLICATED 223 /* Item duplicated */
#define IDX_ERR_DIVIDE_BY_ZERO 224  /* Divide by zero */
#define IDX_ERR_REQUIRE_MINI_PROGRAM_RUNNING                                   \
  225 /* This operation requires mini program running */
#define IDX_ERR_FORM_NOT_EXIST 226 /* Form not exists */
#define IDX_ERR_CANNOT_CONFIG_WHEN_DEVICE_RUNNING                              \
  227 /* Can not config when the device is running */
...
#define IDX_ERR_PYTHON_MODULE_NOT_DEPLOYED                                     \
  232 /* Cannot call API because Python module not deployed */
...
#define IDX_ETH_DIAG_POWER_MODE_RES                                            \
  239                                   /* Response of Diagnostic Power Mode   \
                                         */
...
#define IDX_SOCKET_PREPARE_ENVIROMENT_FIRST                                    \
  250                         /* Please prepare enviroment in TSMaster first */
...
#define IDX_GP_CANNOT_INSERT_GOTO_BETWEEN_ACTIONS                              \
  254 /* Graphic program Cannot insert goto between actions */
#define IDX_GP_CANNOT_DELETE_ACTION_WITH_BOTH_DIR                              \
  255 /* Graphic program Cannot delete action with both directions */
#define IDX_GP_ENTRY_POINT_CANNOT_BE_DELETED                                   \
  256 /* Graphic program entry point cannot be deleted */
#define IDX_GP_KIND_CANNOT_BE_CHANGED                                          \
  257 /* Graphic program action type cannot be changed */
#define IDX_GP_INCORRECT_ACTION_TYPE                                           \
  258 /* Graphic program incorrect action type being edited */
#define IDX_GP_INCORRECT_EXECUTION_KIND                                        \
  259 /* Graphic program incorrect execution kind being edited */
#define IDX_GP_ACTION_GROUP_REQUIRED                                           \
  260 /* Graphic program action group required */
#define IDX_GP_CANNOT_ADD_DOWNWARD_ACTION                                      \
  261 /* Graphic program cannot add downward action */
#define IDX_GP_CANNOT_ADD_RIGHTWARD_ACTION                                     \
  262 /* Graphic program cannot add rightward action */
#define IDX_RBS_NODE_SIMULATION_IS_NOT_ACTIVE                                  \
  263                                    /* RBS node simulation is not active */
...
#define IDX_GPG_EXCEL_FORMAT_INVALID                                           \
  266 /* Graphic program generator excel format invalid */
#define IDX_GPG_EXCEL_UNKNOWN_OBJ                                              \
  267 /* Graphic program generator excel unknown object*/
#define IDX_GPG_EXCEL_OBJ_NOT_FOUND                                            \
  268 /* Graphic program generator excel object not found */
#define IDX_GPG_EXCEL_OBJ_NOT_DEFINED                                          \
  269 /* Graphic program generator excel object not defined */
...
#define IDX_ERR_IP_FRAGMENTATION_NEED                                          \
  273 /* Fragmentation is needed when building IPv4 packet */
...
#define IDX_ERR_WRITE_DEVICE_INT_CONFIG_FAILED                                 \
  276 /* Write internal configuration of Device(such as TC1011 Etc) Failed */
#define IDX_ERR_READ_DEVICE_INT_CONFIG_FAILED                                  \
  277 /* Read internal configuration of Device(such as TC1011 Etc) Failed */
#define IDX_ERR_ARGUMENT_COUNT_DIFFER                                          \
  278                                      /* API caller argument count differ \
                                            */
#define IDX_ERR_API_CALLER_CALL_FAILED 279 /* API caller call API failed */
#define IDX_ERR_ETH_FRAME_IS_NOT_IP                                            \
  280 /* Current ethernet frame is not IP frame */
#define IDX_ERR_ETH_FRAME_IS_NOT_TCP                                           \
  281 /* Current ethernet frame is not TCP frame */
#define IDX_ERR_ETH_FRAME_IS_NOT_UDP                                           \
  282 /* Current ethernet frame is not UDP frame */
#define IDX_ERR_ETH_FRAME_DOES_NOT_CONTAIN_CRC                                 \
  283 /* Current ethernet frame does not contain CRC */
#define IDX_ERR_ETH_API_REQUIRES_SINGLE_FRAME                                  \
  284 /* This API requires single frame, not fragment frame */
#define IDX_ERR_ITEM_NOT_ENABLED                                               \
  285 /* This item is not enabled in configuration */
#define IDX_ERR_ITEM_CONFIGURATION_NOT_VALID                                   \
  286                          /* This item configuration is not valid */
#define IDX_ERR_RESERVED01 287 /* Reserved definition */
#define IDX_ERR_SHOULD_START_BATCH_FIRST                                       \
  288 /* Should start batch first before this operation */
#define IDX_ERR_TEST_SYSTEM_MODULE_NOT_LOADED                                  \
  289                                     /* Test system module not loaded */
...
#define IDX_ERR_LANG_KEY_NOT_FOUND                                             \
  293 /* Language item not found ini ini file */
#define IDX_ERR_MAC_ADDRESS_NOT_EXITS                                          \
  294 /* Mac Address not exists in router table */
#define IDX_ERR_IP_PORT_NOT_EXISTS                                             \
  295 /* IP and Port not exists in router table */

// Software Constants
#define CONST_LOG_ERROR 1 /* error message will be displayed with red color */
#define CONST_LOG_WARNING                                                      \
  2                    /* warning message will be displayed with blue color */
#define CONST_LOG_OK 3 /* OK message will be displayed with green color */
#define CONST_LOG_HINT                                                         \
  4 /* hint message will be displayed with yellow color                        \
     */
#define CONST_LOG_INFO                                                         \
  5 /* info. message will be displayed with black color                        \
     */
#define CONST_LOG_VERBOSE                                                      \
  6                   /* verbose message will be displayed with gray color */
...
#define CONST_AM_NOT_RUN                                                       \
  0 /* Graphic Program (Automation Module) Running State - not running state   \
     */
#define CONST_AM_PREPARE_RUN                                                   \
  1 /* Graphic Program (Automation Module) Running State - Prepare running     \
       state */
#define CONST_AM_RUNNING                                                       \
  2 /* Graphic Program (Automation Module) Running State - Running state */
#define CONST_AM_PAUSED                                                        \
  3 /* Graphic Program (Automation Module) Running State - Paused state */
#define CONST_AM_STEPPING                                                      \
  4 /* Graphic Program (Automation Module) Running State - Stepping state */
#define CONST_AM_FINISHED                                                      \
  5 /* Graphic Program (Automation Module) Running State - Finished state */
...
#define CONST_SIGNAL_TYPE_SYSTEM_VAR                                           \
  2 /* Signal Checker - TSignalType.stSystemVar */
#define CONST_SIGNAL_TYPE_FLEXRAY                                              \
  3 /* Signal Checker - TSignalType.stFlexRay                                  \
     */
#define CONST_SIGNAL_TYPE_ETHERNET                                             \
  4 /* Signal Checker - TSignalType.stEthernet */
#define CONST_SIGNAL_CHECK_ALWAYS                                              \
  0 /* Signal Checker - TSignalCheckKind.sckAlways */
#define CONST_SIGNAL_CHECK_APPEAR                                              \
  1 /* Signal Checker - TSignalCheckKind.sckAppear */
#define CONST_SIGNAL_CHECK_STATISTICS                                          \
  2 /* Signal Checker - TSignalCheckKind.sckStatistics */
#define CONST_SIGNAL_CHECK_RISING_EDGE                                         \
  3 /* Signal Checker - TSignalCheckKind.sckRisingEdge */
#define CONST_SIGNAL_CHECK_FALLING_EDGE                                        \
  4 /* Signal Checker - TSignalCheckKind.sckFallingEdge */
#define CONST_SIGNAL_CHECK_MONOTONY_RISING                                     \
  5 /* Signal Checker - TSignalCheckKind.sckMonotonyRising */
#define CONST_SIGNAL_CHECK_MONOTONY_FALLING                                    \
  6 /* Signal Checker - TSignalCheckKind.sckMonotonyFalling */
#define CONST_SIGNAL_CHECK_FOLLOW                                              \
  7 /* Signal Checker - TSignalCheckKind.sckFollow */
#define CONST_SIGNAL_CHECK_JUMP                                                \
  8 /* Signal Checker - TSignalCheckKind.sckJump */
#define CONST_SIGNAL_CHECK_NO_CHANGE                                           \
  9 /* Signal Checker - TSignalCheckKind.sckNoChange */
#define CONST_STATISTICS_MIN                                                   \
  0 /* Signal Checker - TSignalStatisticsKind.sskMin */
#define CONST_STATISTICS_MAX                                                   \
  1 /* Signal Checker - TSignalStatisticsKind.sskMax */
#define CONST_STATISTICS_AVERAGE                                               \
  2 /* Signal Checker - TSignalStatisticsKind.sskAverage */
#define CONST_STATISTICS_STANDARD_DEVIATION                                    \
  3 /* Signal Checker - TSignalStatisticsKind.sskStdDeviation */
#define CONST_SYMBOL_MAPPING_DIR_BIDIRECTION                                   \
  0 /* Symbol mapping direction - bidirection */
#define CONST_SYMBOL_MAPPING_DIR_SGN_TO_SYSVAR                                 \
  1 /* Symbol mapping direction - from signal to sys var */
#define CONST_SYMBOL_MAPPING_DIR_SYSVAR_TO_SGN                                 \
  2 /* Symbol mapping direction - from sys var to signal */
...
#define CONST_SYSTEM_VAR_TYPE_UINT8ARRAY                                       \
  4 /* System Var Type - svtUInt8Array                                         \
     */
#define CONST_SYSTEM_VAR_TYPE_INT32ARRAY                                       \
  5 /* System Var Type - svtInt32Array                                         \
     */
#define CONST_SYSTEM_VAR_TYPE_INT64ARRAY                                       \
  6                                    /* System Var Type - svtInt64Array      \
                                        */
#define CONST_SYSTEM_VAR_TYPE_DOUBLE 7 /* System Var Type - svtDouble */
#define CONST_SYSTEM_VAR_TYPE_DOUBLEARRAY                                      \
  8                                    /* System Var Type - svtDoubleArray */
#define CONST_SYSTEM_VAR_TYPE_STRING 9 /* System Var Type - svtString */
#define CONST_CANFD_CONTROLLER_TYPE_CAN                                        \
  0 /* CAN FD Controller Type - fdtCAN                                         \
     */
#define CONST_CANFD_CONTROLLER_TYPE_ISOCANFD                                   \
  1 /* CAN FD Controller Type - fdtISOCANFD */
#define CONST_CANFD_CONTROLLER_TYPE_NonISOCANFD                                \
  2 /* CAN FD Controller Type - fdtNonISOCANFD */
#define CONST_CANFD_CONTROLLER_MODE_NORMAL                                     \
  0 /* CAN FD Controller Mode - fdmNormal */
#define CONST_CANFD_CONTROLLER_MODE_ACKOFF                                     \
  1 /* CAN FD Controller Mode - fdmACKOff */
#define CONST_CANFD_CONTROLLER_MODE_RESTRICTED                                 \
  2 /* CAN FD Controller Mode - fdmRestricted */
#define CONST_ONLINE_REPLAY_TIMEING_IMMEDIATELY                                \
  0 /* Online replay timing mode - ortImmediately */
#define CONST_ONLINE_REPLAY_TIMEING_ASLOG                                      \
  1 /* Online replay timing mode - ortAsLog */
#define CONST_ONLINE_REPLAY_TIMEING_DELAYED                                    \
  2 /* Online replay timing mode - ortDelayed */
#define CONST_ONLINE_REPLAY_STATUS_NOTSTARTED                                  \
  0 /* Online replay status - orsNotStarted */
#define CONST_ONLINE_REPLAY_STATUS_RUNNING                                     \
  1 /* Online replay status - orsRunning */
#define CONST_ONLINE_REPLAY_STATUS_PAUSED                                      \
  2 /* Online replay status - orsPaused */
#define CONST_ONLINE_REPLAY_STATUS_COMPLETED                                   \
  3 /* Online replay status - orsCompleted */
#define CONST_ONLINE_REPLAY_STATUS_TERMINATED                                  \
  4                              /* Online replay status - orsTerminated */
...
#define CONST_BLF_OBJECT_REALTIMECOMMENT                                       \
...
#define CONST_RBS_INIT_VALUE_OPTIONS_USEDB                                     \
  0 /* RBS Init Value Options - rivUseDB */
#define CONST_RBS_INIT_VALUE_OPTIONS_USELAST                                   \
  1 /* RBS Init Value Options - rivUseLast */
#define CONST_RBS_INIT_VALUE_OPTIONS_USE0                                      \
  2 /* RBS Init Value Options - rivUse0 */
#define CONST_AM_SIGNAL_TYPE_CANSIGNAL                                         \
  0 /* Graphic Program (Automation Module) Signal Type - lastCANSignal */
#define CONST_AM_SIGNAL_TYPE_LINSIGNAL                                         \
  1 /* Graphic Program (Automation Module) Signal Type - lastLINSignal */
#define CONST_AM_SIGNAL_TYPE_SYSVAR                                            \
  2 /* Graphic Program (Automation Module) Signal Type - lastSysVar */
#define CONST_AM_SIGNAL_TYPE_LOCALVAR                                          \
  3 /* Graphic Program (Automation Module) Signal Type - lastLocalVar */
#define CONST_AM_SIGNAL_TYPE_CONST                                             \
  4 /* Graphic Program (Automation Module) Signal Type - lastConst */
#define CONST_AM_SIGNAL_TYPE_FLEXRAYSIGNAL                                     \
  5 /* Graphic Program (Automation Module) Signal Type - lastFlexRaySignal */
#define CONST_AM_SIGNAL_TYPE_IMMEDIATEVALUE                                    \
  6 /* Graphic Program (Automation Module) Signal Type - lastImmediateValue */
#define CONST_MP_FUNCTION_SOURCE_SYSTEMFUNC                                    \
  0 /* Mini Program Function Source - lmfsSystemFunc */
#define CONST_MP_FUNCTION_SOURCE_MPLIB                                         \
  1 /* Mini Program Function Source - lmfsMPLib */
#define CONST_MP_FUNCTION_SOURCE_INTERNAL                                      \
  2 /* Mini Program Function Source - lmfsInternal */
#define CONST_MP_VAR_TYPE_INTEGER                                              \
  0 /* Mini Program Variable Type - lvtInteger */
#define CONST_MP_VAR_TYPE_DOUBLE                                               \
  1 /* Mini Program Variable Type - lvtDouble                                  \
     */
#define CONST_MP_VAR_TYPE_STRING                                               \
  2 /* Mini Program Variable Type - lvtString                                  \
     */
#define CONST_MP_VAR_TYPE_CANMSG                                               \
  3 /* Mini Program Variable Type - lvtCANMsg                                  \
     */
#define CONST_MP_VAR_TYPE_CANFDMSG                                             \
  4 /* Mini Program Variable Type - lvtCANFDMsg */
#define CONST_MP_VAR_TYPE_LINMSG                                               \
  5 /* Mini Program Variable Type - lvtLINMsg                                  \
     */
#define CONST_STIM_SIGNAL_STATUS_STOPPED                                       \
  0 /* STIM Signal Status - sssStopped                                         \
     */
#define CONST_STIM_SIGNAL_STATUS_RUNNING                                       \
  1                                       /* STIM Signal Status - sssRunning   \
                                           */
#define CONST_STIM_SIGNAL_STATUS_PAUSED 2 /* STIM Signal Status - sssPaused */
#define CONST_SIGNAL_TESTER_NO_ERROR 0    /* No Error */
#define CONST_SIGNAL_TESTER_CHECK_SIGNAL_NOT_EXISTS_IN_DB                      \
  1 /* Check signal not exists in DB */
#define CONST_SIGNAL_TESTER_MIN_BIGGER_THAN_MAX                                \
  2 /* Minimum value larger than Maximum value */
#define CONST_SIGNAL_TESTER_STAR_TTIME_BIGGER_THAN_END_TIME                    \
  3 /* Start Time larger than End Time */
#define CONST_SIGNAL_TESTER_TRIGGER_MIN_BIGGER_THAN_MAX                        \
  4 /* Trigger Minimum value larger than maximum value */
#define CONST_SIGNAL_TESTER_SIGNAL_COUNT_IS_0                                  \
  5 /* Check signal count is zero                                              \
     */
#define CONST_SIGNAL_TESTER_FOLLOW_SIGNAL_NOT_EXISTS_IN_DB                     \
  6 /* Follow signal not exists in DB */
#define CONST_SIGNAL_TESTER_TRIGGER_SIGNAL_NOT_EXISTS_IN_DB                    \
  7 /* Trigger signal not exists in DB */
#define CONST_SIGNAL_TESTER_SIGNAL_FOLLOW_VIOLATION                            \
  8 /* Signal follow violation */
#define CONST_SIGNAL_TESTER_SIGNAL_MONOTONY_RISING_VIOLATION                   \
  9 /* Signal Monotony Rising voilation */
#define CONST_SIGNAL_TESTER_SIGNAL_MONOTONY_FALLING_VIOLATION                  \
  10 /* Signal Monotony Falling violation */
#define CONST_SIGNAL_TESTER_SIGNAL_NO_CHANGE_VIOLATION                         \
  11 /* Signal no change violation */
#define CONST_SIGNAL_TESTER_SIGNAL_VALUEOUTOFRANGE                             \
  12 /* Signal value out of range */
#define CONST_SIGNAL_TESTER_CAN_SIGNAL_NOT_EXISTS                              \
  13 /* CAN signal not exists                                                  \
      */
#define CONST_SIGNAL_TESTER_LIN_SIGNAL_NOT_EXISTS                              \
  14 /* LIN signal not signals */
#define CONST_SIGNAL_TESTER_FLEXRAY_SIGNAL_NOT_EXISTS                          \
  15 /* FlexRay signal not exists */
#define CONST_SIGNAL_TESTER_SYSTEM_VAR_NOT_EXISTS                              \
  16 /* System variable not exists */
#define CONST_SIGNAL_TESTER_START_FAILED_DUE_TO_INVALID_CONF                   \
  17 /* Signal tester start failed due to invalid configuration */
#define CONST_SIGNAL_TESTER_SIGNAL_VALUE_NOT_EXISTS                            \
  18 /* Check Signal value not exist */
#define CONST_SIGNAL_TESTER_STATISTICS_CHECK_VIOLATION                         \
  19 /* Statistics value not within required rang */
#define CONST_SIGNAL_TESTER_TRIGGER_VALUE_NOT_EXISTS                           \
  20 /* Trigger signal value not exist */
#define CONST_SIGNAL_TESTER_FOLLOW_VALUE_NOT_EXISTS                            \
  21 /* Follow signal value not exist */
#define CONST_SIGNAL_TESTER_TRIGGER_VALUE_NEVER_IN_RANGE                       \
  22 /* Trigger signal value never in rang */
#define CONST_SIGNAL_TESTER_TIME_RANGE_NOT_TOUCHED                             \
  23 /* Allowed time range not touche */
#define CONST_SIGNAL_TESTER_RISING_NOT_DETECTED                                \
  24 /* Check signal rising edge not detecte */
#define CONST_SIGNAL_TESTER_FALLING_NOT_DETECTED                               \
  25 /* Check signal falling edge not detecte */
#define CONST_SIGNAL_TESTER_NOT_APPEARED                                       \
  26 /* Check Signal not appeared in rang */
#define CONST_SIGNAL_TESTER_JUMP_NOT_DETECTED                                  \
  27 /* Check signal jump not detect */
